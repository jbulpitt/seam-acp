...
const PresetFieldSchema = (value) => z.object({ value, locked: z.boolean().optional().default(false) });
const PresetsFileSchema = z.record(z.string().regex(/^\d+$/, "preset key must be a numeric Discord channel id"), z.object({
...
function loadChannelPresets(file) {
    ...
    for (const [channelId, preset] of Object.entries(result.data)) {
        ...
        const normalized = {};
        if (preset.agent)
            normalized.agent = preset.agent;
        if (preset.model)
            normalized.model = preset.model;
        if (preset.cwd)
            normalized.cwd = { ...preset.cwd, value: path.resolve(preset.cwd.value) };

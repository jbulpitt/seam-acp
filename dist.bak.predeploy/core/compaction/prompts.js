...
// model reads them.)
// ---------------------------------------------------------------------------
const JSON_ONLY = "Provide your analysis as a single JSON object with the structure shown below (no surrounding prose or code fences):";
...
export function parseJsonOutput(raw) {
    ...
    if (best.chosen !== undefined)
        return best.chosen;
    throw new Error(`parseJsonOutput: no parseable JSON object found (last error: ${best.lastErr || raw1.lastErr || "none"}). raw[0..200]: ${raw.slice(0, 200)}`);
}
...
function balancedEndIn(text, from) {
...
export const JSON_REPARSE_INSTRUCTION = "Your previous reply could not be parsed as JSON. Re-emit the SAME analysis as a single valid JSON object and nothing else. Critically: inside every string value, escape all double-quotes as \\\" and all newlines as \\n. Begin your reply with `{`.";
...
export function mergePinnedFacts(parts) {
    const keys = [
        "corrections", "constraints", "decisions", "openTodos", "activePaths", "rules",
    ];
    const out = { corrections: [], constraints: [], decisions: [], openTodos: [], activePaths: [], rules: [] };
...
export function pinnedFactsPrompt(args) {
    return `${ANALYST_FRAME}
...
// ---------------------------------------------------------------------------
export function preWarmFraming() {
    return [

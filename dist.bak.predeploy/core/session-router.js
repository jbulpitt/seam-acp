...
export class SessionRouter {
    ...
    async startRuntime(record) {
        ...
        if (!profile) {
            throw new Error(`Unknown agent profile "${record.agentId}" for session ${record.id}`);
        }
        ...
        if (record.acpSessionId) {
            try {
                ...
                this.logger.debug({ sessionId: record.id, acpSessionId: record.acpSessionId }, "resumed acp session");
                return runtime;
            }
            catch (err) {
                this.logger.warn({ err, sessionId: record.id }, "session/load failed, creating new session");
            }
        }

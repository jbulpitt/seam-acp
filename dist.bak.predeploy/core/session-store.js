...
export class SessionStore {
    ...
    upsertAlias(a) {
        this.db
            .prepare(`INSERT INTO thread_aliases (alias, channel_ref, description, created_by, created_utc)
    ...
    getAlias(alias) {
        const row = this.db
            .prepare("SELECT * FROM thread_aliases WHERE alias = ? COLLATE NOCASE")
            .get(alias);
        return row ? mapAlias(row) : null;
    }
    listAliases() {
        return this.db
            .prepare("SELECT * FROM thread_aliases ORDER BY alias ASC")
            .all()
            .map(mapAlias);
    }
    deleteAlias(alias) {

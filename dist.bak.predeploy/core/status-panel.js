export function renderStatusPanel(renderer, input, nowUtc) {
    const elapsedSeconds = Math.max(0, Math.floor((nowUtc - input.startedUtc) / 1000));
    const panel = {
        state: input.state,
        elapsedSeconds,
...
export class TurnStatus {
    ...
    isSystemInstruction(text) {
        const lower = text.toLowerCase();
        return (lower.includes("tool specificity") ||
    ...
    pushThinkingChunk(text) {
        ...
        for (const raw of complete.split("\n")) {
            const t = raw.trim();
            if (!t || this.isSystemInstruction(t))
                continue;
    ...
    thinkingWindow() {
        ...
        if (this.thinkingLines.length === 0 && !showPending)
            return undefined;

 ...
 */
export function findFirstUnsafeIndex(buf) {
    const stack = [];
    let i = 0;
    const n = buf.length;
    while (i < n) {
        ...
        if (ch === 92) {
            let run = 1;
            while (i + run < n && buf.charCodeAt(i + run) === 92)
                run++;
            i += run;
            if (run % 2 === 1 && i < n)
                i += 1;
 ...
 */
function isInsideInlineCode(buf, pos) {
    let inCode = false;
    let i = 0;
    while (i < pos) {
        if (buf.charCodeAt(i) === 96 /* ` */) {
            ...
            let run = 1;
            while (i + run < buf.length && buf.charCodeAt(i + run) === 96)
                run++;
            if (run === 1) {
...
function findCleanSplit(buf, minIdx, maxIdx, paragraphOnly = false) {
    ...
    // Scan backwards so we find the latest one that is not inside a code span.
    {
        ...
        while (true) {
            ...
            if (para < minIdx)
                break;
            if (!isInsideInlineCode(window, para))
                return { idx: para, skip: 2 };
    ...
    // Line break — find the last one that is not inside an inline-code span.
    {
        ...
        while (true) {
            ...
            if (nl < minIdx)
                break;
            if (!isInsideInlineCode(window, nl))
                return { idx: nl, skip: 1 };
...
function findThematicBreakCut(buf) {
    const re = /(^|\n)[ \t]*-{3}[ \t]*\n/g;
    let m;
    while ((m = re.exec(buf)) !== null) {
        const lineStart = m.index + (m[1] ? m[1].length : 0);
        if (isInsideInlineCode(buf, lineStart))
            continue;
        return { from: m.index, to: m.index + m[0].length };
    }
    return null;
}
export function splitForFlush(buffer, opts) {
    if (!buffer)
        return null;
    ...
    if (!force) {
        ...
        if (unsafeIdx !== -1 && split.idx > unsafeIdx)
            return null;
    ...
    if (buffer.length <= maxLen) {
        ...
        if (unsafeIdx === 0)
            return null; // nothing safe to send yet
    ...
    const windowMax = unsafeIdx === -1 ? maxLen : Math.min(maxLen, unsafeIdx);
    const split = windowMax > minSplit
...
//# sourceMappingURL=stream-flush.js.map
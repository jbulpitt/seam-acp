...
import { MessageFlags, EmbedBuilder, ButtonBuilder, ActionRowBuilder, ButtonStyle, StringSelectMenuBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, AttachmentBuilder } from "discord.js";
import { IMAGE_MODELS, getImageModelById, generateImage, resolveGoogleApiKey, } from "../../core/image-gen.js";
import { serializePanelText } from "../renderer.js";
import { AgentRuntime } from "../../agents/agent-runtime.js";
import { cleanTextForPreview } from "../../agents/session-manager.js";
import { readRichHistory } from "../../core/compaction/source-reader.js";
import { analyzeSessionCoverage, detectGaps } from "../../core/compaction/gap-detector.js";
import { runPremiumCompaction } from "../../core/compaction/pipeline.js";
import { pinnedFactsPrompt, parseJsonOutput, mergePinnedFacts, assembleNewSession } from "../../core/compaction/prompts.js";
import { loadScheduledAttachments, deleteScheduledAttachmentDir, saveScheduledAttachment, deleteScheduledAttachment, } from "../../core/scheduled-prompts/attachments.js";
...
import { ATTACH_FENCE_LANG, DELEGATE_FENCE_PREFIX, withHarnessPreamble } from "../../core/agent-conventions.js";
...
import { FenceStream } from "../../core/fence-stream.js";
...
export class Orchestrator {
    ...
    async handleThreadDeleted(channelRef) {
    ...
    async handleRestartSentinel() {
        ...
        if (this.activeTurns > 0) {
            ...
            await this.postNotification(`♻️ Restart requested — waiting for ${this.activeTurns} ${turnWord} to finish.`);
    ...
    async handleIncomingMessage(msg) {
        ...
        const newQueue = existingQueue.then(async () => {
            ...
            if ((this.channelGenerations.get(channelId) ?? 0) > myGen)
                return;
            this.activeTurns++;
            try {
    ...
    async handleIncomingMessageInner(msg) {
        const channel = msg.channel;
        const record = this.router.ensureSessionRecord({
            platform: channel.platform,
            channelRef: channel.id,
        ...
        const modelContextFloor = turnProfile?.staticModels?.find((m) => m.modelId === activeModel)?.contextLimit ?? 0;
        if (cachedUsage &&
        ...
        const HARD_MAX = 1800;
        const SOFT_MIN = 800;
        const drainBuffer = async (force, allowUnsafeCut = false) => {
            while (textBuffer) {
        ...
        const isSessionGoneError = (e) => {
            const message = e instanceof Error ? e.message : String(e);
            const details = String(e?.data?.details ?? "");
        ...
        const isImageDimensionError = (msg) => /dimension limit for many-image|exceeds the dimension limit/i.test(msg);
        ...
        const isConnectionClosedError = (e) => {
        ...
        const isRateLimitError = (e) => {
            const err = e;
            if (err?.data?.errorKind === "rate_limit")
                return true;
            const msg = e instanceof Error ? e.message : String(e);
        ...
        try {
            ...
            const eventHandler = async (event) => {
                switch (event.kind) {
                    case "agent-text": {
                        ...
                        const isNewMessage = event.messageId !== undefined &&
                        ...
                        if (!loopAborted) {
                            ...
                            const repeatThreshold = loopChunk && loopChunk.length <= LOOP_SHORT_MAX
                                ? LOOP_THRESHOLD_SHORT
                                : LOOP_THRESHOLD_LONG;
                            const repeatTripped = loopChunk !== null && loopCount >= repeatThreshold;
                            const whitespaceTripped = whitespaceRun >= WHITESPACE_RUN_THRESHOLD;
                        ...
                        if (!fenceWatchdogTripped &&
                            fenceStream.inFence &&
                            fenceStream.openSinceMs() > FENCE_MAX_OPEN_MS) {
                            fenceWatchdogTripped = true;
                            this.logger.warn({ session: record.id }, "open fence exceeded watchdog timeout; emitting partial content");
                            const snap = fenceStream.forceClose();
                            if (snap) {
                                fenceCounter += 1;
                                await this.emitClosedFence(channel, snap, fenceCounter, {
                                    preferredRoot: record.repoPath,
                                    notice: "_(fence exceeded the watchdog timeout and was closed early)_",
                                });
                                textSent = true;
                                typingDone = true;
                            }
                    ...
                    case "agent-file": {
                        ...
                        try {
                            ...
                            this.logger.warn({ err, filename: event.filename }, "sendFile failed; falling back to text notice");
                            await this.adapter.sendMessage(channel, `_Agent produced a file (\`${event.filename}\`) but it couldn't be uploaded._`);
                        }
                        return;
                    }
                    case "agent-thought":
                    ...
                    case "usage-update": {
                        ...
                        if (this.config.AGY_AUTO_COMPACT_THRESHOLD > 0 &&
            ...
            if (activeProfile?.restrictDiscordAccess &&
                msg.attachments &&
                msg.attachments.length > 0 &&
                typeof activeProfile.sessionManager?.writeAttachment === "function") {
                const writer = activeProfile.sessionManager.writeAttachment.bind(activeProfile.sessionManager);
                ...
                for (const a of msg.attachments) {
                    try {
                        ...
                        const { path: written } = await writer(cwd, a.filename, buf.toString("base64"));
                        pathLines.push(`- \`${a.filename}\` → \`${written}\``);
                    }
                    catch (err) {
                        this.logger.warn({ err, filename: a.filename }, "failed to write attachment to restricted agent; falling back to skipping");
            ...
            else if (!activeProfile?.restrictDiscordAccess &&
                msg.attachments &&
                msg.attachments.length > 0) {
                    ...
                    for (const backoffMs of [2_000, 5_000, 10_000]) {
                        ...
                        try {
                            ...
                            if (!isRateLimitError(rlErr))
                                throw rlErr; // a different failure — surface it
                        }
                    }
                    if (rlResult === undefined)
                        throw promptErr; // still throttled after backoff
                    result = rlResult;
                }
                else {
                    throw promptErr;
            ...
            this.logger.info({
                ...
                ttftMs: firstChunkAt !== undefined ? firstChunkAt - turnStartedAt : null,
            ...
            if (result !== "timeout" &&
                ...
                const lines = result.rejectedAttachments
                    ...
                    .join("\n");
                await this.adapter.sendMessage(channel, `_Some attachments were not sent to the agent:_\n${lines}`);
            }
            if (!textSent && result !== "timeout" && !result.cancelled) {
            ...
            if (result !== "timeout" && !result.cancelled) {
                ...
                if (usageReader) {
                    try {
                        const cwd = record.repoPath ?? process.cwd();
                        const usage = await usageReader.call(profile.sessionManager, cwd, record.acpSessionId || undefined, turnStartedAt || undefined);
                        ...
                        const modelEntry = profile?.staticModels?.find((m) => m.modelId === selectedModel);
            ...
            if (isSessionGoneError(err)) {
                ...
                this.logger.warn({ session: record.id, isPromptTooLong, isImageDimension }, "agent rejected prompt; invalidating session runtime");
                ...
                if (needsRepair) {
                    ...
                    if (manager && typeof manager.repairSession === "function" && record.acpSessionId) {
                        try {
                            this.logger.info({ session: record.id, acpSessionId: record.acpSessionId, reason: isImageDimension ? "image-dimension" : "context-size" }, "auto-repairing session");
                    ...
                    if (repaired && isImageDimension) {
                        await this.adapter.sendMessage(channel, "⚠️ **An image in the conversation exceeded the 2000px many-image limit.** The session was automatically repaired by stripping image payloads from the history (testing screenshots are the usual cause). You can safely retry your message now!");
                    }
                    else if (repaired) {
                        await this.adapter.sendMessage(channel, "⚠️ **Claude hit its context limit before auto-compacting.** The session was automatically repaired by stripping heavy base64 image payloads and rolling back the last incomplete message. You can safely retry your message now!");
                    }
                    else {
                        await this.adapter.sendMessage(channel, "⚠️ **Claude hit its context limit before auto-compacting.** The context grew too large in a single turn. Try running `/compact` to free up space!");
                    }
                }
            }
            status.setState("Failed");
    ...
    async handleSlashInteraction(interaction) {
        ...
        if (interaction.options.getSubcommandGroup(false) === "alias") {
            return this.cmdAlias(interaction);
        }
        ...
        switch (sub) {
            ...
            case "peek":
                return this.cmdPeek(interaction);
            default:
    ...
    async probeCopilotContext(runtime, realHandler, refresh) {
        let captured = "";
        ...
        try {
            await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error("/context probe timed out")), 5_000)),
            ]);
            const m = captured.match(/(\d+(?:\.\d+)?)k\s*\/\s*(\d+(?:\.\d+)?)k\s*tokens/i);
            if (m) {
    ...
    async runAgyAutoCompact(record, channel, status, refresh, tokensBefore) {
        const profile = this.router.getProfile(record.agentId);
        const manager = profile?.sessionManager;
    ...
    makeCompactionRunAgent(profile, manager, opts) {
    ...
    compactionEffortFor(profile, tier) {
        const levels = profile.effort?.levels ?? [];
        if (levels.length === 0)
            return undefined;
        if (levels.includes("xhigh"))
            return tier === "premium" ? "xhigh" : "high";
        return tier === "premium"
    ...
    renderDiscordRanges(msgs, ranges) {
        const inAny = (ts) => ranges.some((r) => {
    ...
    async runPremiumCompactionForSession(args) {
        const { profile, manager, sessionId, cwd, channel, onProgress } = args;
        const log = (m) => { onProgress?.(m); this.logger.debug({ compaction: sessionId }, m); };
        ...
        if (!jsonlPath)
            throw new Error("Could not locate the session's raw history file.");
        ...
        if (channel && typeof this.adapter.fetchThreadMessagesTimed === "function") {
            try {
                threadMsgs = await this.adapter.fetchThreadMessagesTimed(channel);
            }
            catch (err) {
                this.logger.warn({ err }, "premium-compact: thread fetch failed");
            }
        }
    ...
    splitTranscriptTurns(transcript) {
        return transcript
            .split(/\n\n(?=### (?:User|Assistant)\n)/)
            .map((t) => t.trim())
            .filter((t) => t.length > 0);
    ...
    async buildDefaultCompactionSeed(args) {
        ...
        for (let i = turns.length - 1; i >= 0; i--) {
            ...
            if (chars + len > budgetChars && recent.length > 0)
                break;
        ...
        let pinnedFacts = { corrections: [], constraints: [], decisions: [], openTodos: [], activePaths: [], rules: [] };
        try {
            ...
            pinnedFacts = mergePinnedFacts([parseJsonOutput(raw)]);
        }
        catch (err) {
            this.logger.warn({ err, sessionId }, "default-compact: pinned-facts extraction failed; continuing without it");
        }
        const pinnedCount = pinnedFacts.corrections.length + pinnedFacts.constraints.length + pinnedFacts.rules.length +
            pinnedFacts.openTodos.length + pinnedFacts.activePaths.length;
        const dropNote = `## Compaction note\n` +
    ...
    formatPremiumReport(result, sessionId) {
    ...
    async seedNewSession(args) {
        ...
        try {
            ...
            const prompt = "[Loading prior-session context after compaction — read the summary below, reply with a one-line acknowledgement, then await the next instruction. Do not begin work yet.]\n\n" +
                summary;
            await rt.prompt(prompt);
    ...
    async runScheduledPromptInner(row) {
        const id = row.id;
        const bindingThread = {
            platform: PLATFORM,
            id: row.channelRef,
        ...
        const target = row.targetChannel
        ...
        try {
            if (this.adapter.sendPanel)
                await this.adapter.sendPanel(target, running);
            else
                await this.adapter.sendMessage(target, `⏰ Running scheduled prompt "${row.name}"…`);
        }
        catch (err) {
    ...
    async runIsolatedScheduledJob(args) {
        ...
        try {
            ...
            const outcome = await raceWithTimeout(rt.prompt(promptText, attachments.length ? attachments : undefined), this.config.TURN_TIMEOUT_SECONDS * 1000);
            if (outcome === "timeout") {
    ...
    async postScheduledResult(channel, name, text, outputType) {
        ...
        if (outputType === "messages") {
            ...
            if (chunks.length <= 8) {
                for (const c of chunks)
                    await this.adapter.sendMessage(channel, c);
            }
            else {
    ...
    async partitionAndStageAttachments(attachments) {
    ...
    async sendResultCard(channel, title, description, color) {
        const p = { color, title, description: description.slice(0, 4096), fields: [] };
        if (this.adapter.sendPanel)
            await this.adapter.sendPanel(channel, p);
        else
            await this.adapter.sendMessage(channel, `${title}\n${description}`);
    }
    async sendResultFile(channel, name, body) {
        ...
        if (this.adapter.sendFile) {
            ...
            for (const c of this.chunkString(body, 1900))
                await this.adapter.sendMessage(channel, c);
        }
    }
    /** Update last_run/last_status, preserving next_run (manager-owned). */
    patchScheduledStatus(id, status) {
    ...
    async downloadAttachmentBytes(url) {
        const res = await fetch(url);
        if (!res.ok)
            throw new Error(`download failed (${res.status})`);
        return Buffer.from(await res.arrayBuffer());
    }
    scheduleSummaryLine(s) {
        ...
        collector.on("collect", async (c) => {
            try {
                ...
                if (action === "edit") {
                    ...
                    const updated = { ...row, enabled: !row.enabled, updatedUtc: new Date().toISOString() };
                    this.store.upsertScheduled(updated);
                    if (updated.enabled)
                        this.scheduledManager?.armFromRow(updated);
                    else
                        this.scheduledManager?.disarm(id);
    ...
    buildScheduleListMessage(channel) {
        const rows = this.store.listScheduledByChannel(PLATFORM, channel.id);
        ...
        for (const r of rows.slice(0, 5)) {
            components.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`sl:edit:${r.id}`).setLabel(`✏️ ${r.name}`.slice(0, 80)).setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId(`sl:toggle:${r.id}`).setLabel(r.enabled ? "⏸️ Disable" : "🟢 Enable").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(`sl:del:${r.id}`).setLabel("🗑️ Delete").setStyle(ButtonStyle.Danger)));
        }
        return { embeds: [embed], components };
    }
    async cmdScheduleRemove(i) {
    ...
    async cmdScheduleAddFile(i) {
        ...
        try {
            ...
            await i.editReply(`❌ Failed to add file: ${err.message}`);
        }
    }
    async cmdScheduleRemoveFile(i) {
        const id = i.options.getString("id", true);
    ...
    async cmdScheduleAdd(i, existing) {
        const channel = this.channelRefFromInteraction(i);
        if (!channel) {
        ...
        if (i.isChatInputCommand()) {
            for (const opt of ["file", "file2", "file3"]) {
                ...
                if (a)
                    pending.push({ name: a.name, url: a.url, mime: a.contentType ?? "application/octet-stream" });
            }
        }
        const state = {
            name: existing?.name ?? "",
            ...
            outputType: (existing?.outputType ?? "card"),
            files: pending,
        };
        ...
        const render = () => {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(SCHEDULED_COLOR)
                .setDescription("This runs **on its own, on a clean session** — it won't remember this conversation. " +
                "Write the prompt so it stands alone, and attach any files it needs (re-sent every run).")
                .addFields({ name: "🏷️ Name", value: state.name || "*(not set)*" }, { name: "✏️ Prompt", value: state.promptText ? "```\n" + state.promptText.slice(0, 1000) + "\n```" : "*(not set — click ✏️ Prompt & name)*" }, { name: "🕐 Runs", value: cronLine + (next ? `\nNext: <t:${Math.floor(next.getTime() / 1000)}:F>` : ""), inline: true }, { name: "🌍 Timezone", value: state.timezone, inline: true }, { name: "🤖 Model", value: state.model ? `\`${state.model}\`` : `Session default${sessionModel ? ` (\`${sessionModel}\`)` : ""}`, inline: true }, { name: "📂 Working dir", value: state.cwd ? `\`${state.cwd}\`` : "*(this thread's repo)*", inline: true }, { name: "📮 Output to", value: state.target ? `<#${state.target}>` : "*(this thread)*", inline: true }, { name: "🖼️ Output as", value: state.outputType === "messages" ? "plain messages" : "status cards", inline: true }, { name: "📎 Files", value: filesValue });
            ...
            const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("sched:prompt").setLabel("✏️ Prompt & details").setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId("sched:output").setLabel(state.outputType === "messages" ? "🖼️ Output: messages" : "🖼️ Output: cards").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId("sched:create").setLabel(existing ? "💾 Save" : "✅ Create").setStyle(ButtonStyle.Success).setDisabled(!state.cron || !state.promptText || !state.name), new ButtonBuilder().setCustomId("sched:cancel").setLabel("Cancel").setStyle(ButtonStyle.Secondary));
            const rows = [
                new ActionRowBuilder().addComponents(cadence),
                new ActionRowBuilder().addComponents(tz),
            ];
            if (models.length > 0) {
                const modelSelect = new StringSelectMenuBuilder()
                    .setCustomId("sched:model")
                    .setPlaceholder("🤖 Model")
                    .addOptions({ label: `Session default${sessionModel ? ` (${sessionModel})` : ""}`.slice(0, 100), value: "__default__", default: state.model === null }, ...models.map((m) => ({ label: m.name.slice(0, 100), value: m.modelId, default: m.modelId === state.model })));
                rows.push(new ActionRowBuilder().addComponents(modelSelect));
            }
            rows.push(buttons);
            ...
            if (existing && editFiles.length > 0) {
                ...
                rows.push(new ActionRowBuilder().addComponents(rmfile));
            }
                ...
                if (c.isStringSelectMenu() && c.customId === "sched:tz") {
                    ...
                    if (v === "__custom__") {
                        const modal = new ModalBuilder().setCustomId(`sched:cronmodal:${msg.id}`).setTitle("Custom schedule")
                            .addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("cron").setLabel("Cron expression (min hour dom mon dow)")
                    ...
                    const modal = new ModalBuilder().setCustomId(`sched:promptmodal:${msg.id}`).setTitle("Prompt & details")
                        .addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("name").setLabel("Name").setStyle(TextInputStyle.Short).setRequired(true).setValue(state.name).setMaxLength(80)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("prompt").setLabel("Prompt (stands on its own — no prior context)")
                        .setStyle(TextInputStyle.Paragraph).setRequired(true).setValue(state.promptText)
                        .setPlaceholder("e.g. Run `npm test`, then post any failures as file:line with a one-line fix.")), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("cwd").setLabel("Working dir (optional)").setStyle(TextInputStyle.Short).setRequired(false).setValue(state.cwd ?? "")
                        .setPlaceholder("blank = this thread's repo; or a path under REPOS_ROOT")), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("target").setLabel("Output channel/thread id (optional)").setStyle(TextInputStyle.Short).setRequired(false).setValue(state.target ?? "")
                    ...
                    if (sub) {
                        ...
                        if (rawCwd) {
                            try {
                                state.cwd = resolveRepoPath(this.config.REPOS_ROOT, rawCwd);
                            }
                            catch (e) {
                                errors.push(`cwd: ${e.message}`);
                            }
                        }
                        else
                            state.cwd = null;
                        ...
                        if (rawTarget) {
                            if (/^\d+$/.test(rawTarget))
                                ...
                                errors.push("output id must be a numeric channel/thread id");
                        }
                        else
                            state.target = null;
                        await sub.deferUpdate();
                        await i.editReply(render());
                        if (errors.length)
                            await sub.followUp({ content: `⚠️ ${errors.join("; ")}`, flags: MessageFlags.Ephemeral });
                    }
                }
                    ...
                    if (!state.name || !state.promptText || !state.cron) {
                        ...
                        if (!state.name)
                            missing.push("a name");
                        if (!state.promptText)
                            missing.push("a prompt");
                        if (!state.cron)
                            missing.push("a cadence/schedule");
                        await c.followUp({
    ...
    async cmdNew(i) {
        ...
        await i.deferReply({ flags: MessageFlags.Ephemeral });
        const parent = { platform: PLATFORM, id: i.channelId };
        ...
        try {
            ...
            await i.editReply(`Created thread <#${thread.id}>. Run \`/seam init\` there to begin.`);
        }
    }
    async cmdRepo(i) {
        const channel = this.channelRefFromInteraction(i);
        ...
        try {
            ...
            await i.reply({
                content: `Invalid path: ${err.message}`,
                flags: MessageFlags.Ephemeral,
            });
            return;
        }
        ...
        if (!id) {
            ...
            if (profile?.staticModels && profile.staticModels.length > 0) {
                ...
                try {
                    ...
                    await i.editReply(`Current model: ${displayCurrent}\nFailed to start the agent to list models: ${err.message}`);
                    return;
                }
            }
            if (models.length === 0) {
     ...
     */
    async applyModelChange(channel, record, id, interaction) {
        const cfg = this.store.readConfig(record);
        cfg.model = id;
    ...
    async cmdEffort(i) {
        ...
        if (supported.length === 0) {
            const msg = eff?.mechanism === "modelBaked"
        ...
        if (!level) {
            ...
            if (!channel || !this.adapter.sendChoicePicker) {
                const body = cfg.reasoningEffort
    ...
    async applyEffortChange(record, level) {
        const cfg = this.store.readConfig(record);
    ...
    async cmdCancel(i) {
        ...
        await i.editReply(outcome === "idle" ? "No active turn." :
    ...
    async cmdImage(i) {
        ...
        if (!prompt.trim()) {
            ...
            this.attachImagePickerCollector(msg, state);
            return;
        }
        await i.deferReply();
        const state = this.makePickerState(prompt);
    ...
    makePickerState(prompt) {
        const defaultModel = getImageModelById("nano-banana-2") ?? IMAGE_MODELS[0];
        return {
            prompt,
    ...
    attachImagePickerCollector(msg, state) {
        ...
        collector.on("collect", async (bi) => {
            ...
            try {
                ...
                if (customId === "img:model") {
                    ...
                    if (state.count > model.maxCount)
                        state.count = model.maxCount;
                    await bi.update(renderImagePicker(state));
                    return;
                }
                if (customId.startsWith("img:aspect:")) {
                    state.aspectRatio = customId.slice("img:aspect:".length);
                    await bi.update(renderImagePicker(state));
                    return;
                }
                ...
                if (customId.startsWith("img:res:")) {
                    state.resolution = customId.slice("img:res:".length);
                    await bi.update(renderImagePicker(state));
                    return;
                }
                ...
                if (!bi.replied && !bi.deferred) {
                    await bi.reply({ content: `Picker error: ${err.message}`, flags: MessageFlags.Ephemeral }).catch(() => { });
                }
            }
        });
        collector.on("end", () => {
    ...
    async handleImageRefsButton(bi, state) {
    ...
    async handleImageGenerate(bi, state, msg) {
        const model = getImageModelById(state.modelId);
        if (!model) {
            await bi.reply({ content: "Model not found.", flags: MessageFlags.Ephemeral });
            return;
        }
        const googleApiKey = await resolveGoogleApiKey(this.config.GOOGLE_AI_STUDIO_API_KEY, this.config.GOOGLE_AI_STUDIO_API_KEY_FILE);
        ...
        try {
            ...
            await bi.editReply(renderImagePicker(state, {
                status: "error",
        ...
        const parentId = msg.channel?.parentId ?? undefined;
        const record = this.router.ensureSessionRecord({
            platform: "discord",
        ...
        try {
            await bi.followUp({
                content: `**${model.displayName}** · ${state.aspectRatio} · ${result.images.length} image${result.images.length === 1 ? "" : "s"} · ${elapsed}s` +
                    (savedPaths.length > 0 ? `\n\`${savedPaths[0]}\`` : ""),
                files,
            });
        }
    ...
    async applyAgentChange(channel, record, id, interaction) {
        const profile = this.router.getProfile(id);
        if (!profile) {
    ...
    async cmdConfig(i) {
        ...
        const cfg = this.store.readConfig(record) ?? defaultSessionConfig(this.config.DEFAULT_MODEL);
        await i.reply({
    ...
    async cmdSessions(i) {
        ...
        const makeSessionMessageOptions = (idx, list, activeId, mgr) => {
            const isOrphaned = !list.some((s) => s.sessionId === activeId);
            if (list.length === 0) {
                const embed = new EmbedBuilder()
                    .setTitle(`Browse & Manage Sessions — ${profile.displayName}`)
                    .setDescription(`⚠️ **Warning:** The current Discord thread is completely disconnected from any known backend session.\n\n` +
                ...
                const row = new ActionRowBuilder().addComponents(rebuildBtn);
                return {
                    content: "",
                    embeds: [embed],
                    components: [row],
            ...
            if (!session)
                return { content: "No sessions found.", embeds: [], components: [] };
            const formatted = session.previewLines.map(formatLine).filter(Boolean);
            const previewText = formatted.length > 0
                ? formatted.join("\n")
                : "*No meaningful messages in this session.*";
            const embed = new EmbedBuilder()
                .setTitle(`Browse & Manage Sessions — ${profile.displayName}`)
                .setDescription((isOrphaned ? `⚠️ **Warning:** The current Discord thread is completely disconnected from any known backend session.\n\n` : "") +
            ...
            const targetProfiles = this.router.listProfiles().filter(p => p.id !== record.agentId && !!p.sessionManager);
            const row1 = new ActionRowBuilder().addComponents(prevBtn, nextBtn, closeBtn);
            const row2 = new ActionRowBuilder().addComponents(attachBtn, cloneBtn, cloneAttachBtn, deleteBtn);
            const row3Buttons = [summaryBtn];
            if (canCompact) {
                row3Buttons.push(new ButtonBuilder()
            ...
            const row4Buttons = [];
            if (canCompact) {
                row4Buttons.push(new ButtonBuilder()
            ...
            if (row4Buttons.length > 0) {
                components.push(new ActionRowBuilder().addComponents(row4Buttons));
            }
            return {
                content: "",
                embeds: [embed],
            ...
            if (customId === "sessions:prev") {
                ...
                if (session) {
                    ...
                    try {
                        ...
                        if (embed) {
                            embed.setDescription(`✨ **Cloned successfully as** \`${newSessionId}\`!\n\n` +
                        ...
                        await btnInteraction.editReply({
                            embeds: [
                                new EmbedBuilder()
                                    ...
                                    .setDescription(`✨ **Cloned successfully as** \`${newSessionId}\`!\n\n` +
                ...
                if (session && typeof manager.repairSession === "function") {
                    try {
                        ...
                        const embed = opts.embeds?.[0];
                        if (embed) {
                            embed.setDescription(`✨ **Session repaired successfully!**\n\n` +
                                (embed.data.description ?? ""));
                        }
                        await btnInteraction.editReply(opts);
                    }
                ...
                void (async () => {
                    ...
                    try {
                        ...
                        sanitizedTranscript = fitTranscriptToWindow(sanitizedTranscript, templateOverhead, compactionWindowFor(compactionModel));
                ...
                if (session) {
                    ...
                    void (async () => {
                        ...
                        try {
                            ...
                            const summaryPrompt = `Please summarize the following conversation session. Highlight:\n` +
                            ...
                            const summaryEmbed = new EmbedBuilder()
                                ...
                                .setDescription(`**Session ID:** \`${session.sessionId}\`\n\n` +
                                `${displaySummary}`)
                                .setColor(0x9b59b6);
                            const row = new ActionRowBuilder().addComponents(new ButtonBuilder()
                                .setCustomId("sessions:summary_back")
                ...
                if (session) {
                    ...
                    const backRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("sessions:summary_back").setLabel("⬅ Back to Manage").setStyle(ButtonStyle.Secondary));
                    void (async () => {
                        try {
                            if (!this.compactionModelFor(record.agentId)) {
                            ...
                            if (!built)
                                throw new Error("Nothing to compact (empty transcript or no summarizer model).");
                            ...
                            const successEmbed = new EmbedBuilder()
                                ...
                                .setDescription(`Compacted into a **new session** \`${newId}\` (summarized ${built.summarizedTurns} older turn(s), kept ${built.keptTurns} verbatim, pinned ${built.pinnedCount} fact(s)).` +
                ...
                if (session) {
                    ...
                    void (async () => {
                        ...
                        let editing = false;
                        const lines = [];
                        const pushProgress = (m) => {
                            lines.push(m);
                            const now = Date.now();
                            if (editing || now - lastEdit < 2500)
                                return;
                            editing = true;
                            lastEdit = now;
                        ...
                        try {
                            ...
                            if (!result.assembledSeed.trim())
                                throw new Error("Pipeline produced an empty result.");
                            ...
                            const successEmbed = new EmbedBuilder()
                                ...
                                .setDescription(`Compacted into a **new session** \`${newId}\` with the multi-agent pipeline.` +
                                (wasActive ? ` This thread is now bound to it.` : ``) +
                                `\nOriginal \`${session.sessionId}\` is **preserved** (review or delete it from this list).`)
                                .addFields({ name: "Chunks", value: String(result.stats.chunks), inline: true }, { name: "Deep-dives", value: String(result.stats.deepDives), inline: true }, { name: "Completeness", value: `${preserved}/${v.length} preserved`, inline: true }, { name: "Recoveries flagged", value: String(result.stats.recoveriesRequested), inline: true })
                                .setColor(0x2ecc71);
                            await btnInteraction.editReply({
                                embeds: [successEmbed],
                                components: [backRow],
                ...
                const submission = await btnInteraction.awaitModalSubmit({
                    filter: (mi) => mi.customId === `sessions:import_cwd_modal:${session.sessionId}` &&
                ...
                try {
                    ...
                    await submission.reply({
                        content: `❌ Invalid cwd: ${err.message}`,
                        flags: MessageFlags.Ephemeral,
                    });
                    return;
                }
                ...
                await submission.editReply({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setDescription(`Summarizing session \`${session.sessionId}\` and creating a new session under \`${this.repoDisplay(targetCwd)}\`…`)
                            .setColor(0xe67e22),
                    ],
                    components: [],
                });
                void (async () => {
                    ...
                    try {
                        ...
                        const successEmbed = new EmbedBuilder()
                            ...
                            .setDescription(`Summary of \`${session.sessionId}\` was seeded into a fresh session.\n\n` +
                        ...
                        if (tempRuntime) {
                            ...
                            if (tempSessionId) {
                                await manager.deleteSession(targetCwd, tempSessionId).catch((cleanupErr) => {
                                    this.logger.warn({ err: cleanupErr, sessionId: tempSessionId }, "failed to clean up temporary import session");
                                });
                            }
                        }
                    }
                ...
                const session = sessions[currentIndex];
                if (session) {
                    const targetProfiles = this.router.listProfiles().filter(p => p.id !== record.agentId &&
                        !!p.sessionManager);
                    const embed = new EmbedBuilder()
                        .setTitle(`Migrate Session — ${profile.displayName}`)
                        .setDescription(`Migrate session \`${session.sessionId}\` to a different agent.\n\n` +
                    ...
                    const select = new StringSelectMenuBuilder()
                        ...
                        .addOptions(targetProfiles.map(p => ({
                        label: p.displayName,
                        value: p.id,
                    ...
                    void (async () => {
                        ...
                        try {
                            ...
                            const successEmbed = new EmbedBuilder()
                                ...
                                .setDescription(`Successfully migrated to agent **${targetProfile.displayName}**.\n\n` +
                                `**New Session ID:** \`${newSessionId}\`\n\n` +
            ...
            try {
                ...
                if (currentSession) {
                    const embed = new EmbedBuilder()
                        ...
                        .setDescription(`**Session ID:** \`${currentSession.sessionId}\`\n` +
    ...
    async cmdTools(i) {
        ...
        const cfg = this.store.readConfig(record);
        if (action === "allow")
            cfg.availableTools = list;
        else if (action === "exclude")
            cfg.excludedTools = list;
        this.persistConfig(record, cfg);
        await this.router.invalidate(record.id);
        await i.reply({
    ...
    async cmdConfigSet(i) {
        ...
        try {
            ...
            await i.reply({
                content: `Invalid JSON: ${err.message}`,
                flags: MessageFlags.Ephemeral,
            });
            return;
        }
    ...
    async cmdApprove(i) {
        ...
        this.persistConfig(record, cfg);
        const messages = {
            always: "Approval policy set to `always`. ⚠️ The agent will auto-approve every permission request (shell exec, file writes, network, etc.).",
            ask: "Approval policy set to `ask`. The bot will post a Discord prompt for each permission request and auto-deny after 5 minutes.",
            deny: "Approval policy set to `deny`. The agent will be auto-denied every permission request — useful for read-only sessions.",
        };
     ...
     */
    async resolveAllowedHostFile(requested, opts = {}) {
        ...
        if (path.isAbsolute(cleaned)) {
            ...
            if (opts.preferredRoot)
                bases.push(path.resolve(opts.preferredRoot));
            for (const r of allowedRoots) {
                if (!bases.includes(r))
                    bases.push(r);
            }
            for (const base of bases)
                candidates.push(path.resolve(base, cleaned));
        }
        for (const candidate of candidates) {
            ...
            if (!this.config.ATTACH_ALLOW_ANY_PATH &&
    ...
    async cmdAttach(i) {
        ...
        if (!resolved) {
            await i.editReply(`Could not attach \`${requested}\` — file not found (relative to the thread's repo or an allowed root), not a regular file, or outside REPOS_ROOT / ATTACH_ROOTS.`);
            return;
        }
        const MAX = 25 * 1024 * 1024;
        if (resolved.size > MAX) {
            await i.editReply(`File too large for Discord: ${resolved.size} B (25 MB limit).`);
            return;
        }
        let data;
        try {
            ...
            await i.editReply(`Read failed: ${err.message}`);
            return;
        }
        ...
        try {
            ...
            await i.editReply(`Upload failed: ${err.message}`);
        }
    }
    async cmdWhoami(i) {
        await i.deferReply({ flags: MessageFlags.Ephemeral });
        ...
        if (!id) {
            await i.editReply({
                content: `Agent \`${profile.id}\` (${profile.displayName}) — no logged-in account found. ` +
    ...
    async cmdUsage(i) {
        ...
        const isCopilot = record.agentId === "copilot" ||
     ...
     */
    async sendAgentFile(channel, event) {
        const buf = event.base64
            ? Buffer.from(event.data, "base64")
            : Buffer.from(event.data, "utf8");
        if (!this.adapter.sendFile) {
            await this.adapter.sendMessage(channel, `_Agent produced \`${event.filename}\` (${event.mimeType}, ${buf.byteLength} B) but this platform doesn't support file uploads._`);
            return;
        }
        const MAX_DISCORD_BYTES = 25 * 1024 * 1024;
        if (buf.byteLength > MAX_DISCORD_BYTES) {
            await this.adapter.sendMessage(channel, `_Agent produced \`${event.filename}\` (${buf.byteLength} B) — too large for Discord (25 MB limit)._${event.uri ? ` Source: ${event.uri}` : ""}`);
            return;
        }
     ...
     */
    async emitClosedFence(channel, fence, counter, opts = {}) {
        ...
        // Cross-thread delegation: a fence tagged `seam-delegate:<alias>` posts
        // the body as a prompt in the target thread, triggering that agent.
        if (fence.lang.startsWith(DELEGATE_FENCE_PREFIX)) {
            await this.emitDelegateFence(channel, fence);
            return;
        }
        // Inline-rendered total size = ```lang\n<content>\n``` plus optional
        // trailing notice on its own paragraph.
        const inlineMessageLen = 3 + fence.lang.length + 1 + fence.content.length + 1 + 3 +
     ...
     */
    async emitAttachFence(channel, fence, opts) {
        ...
        try {
            ...
            this.logger.info({ realPath: resolved.realPath, bytes: data.byteLength }, "seam-attach fence → uploaded workspace file");
        }
        catch (err) {
     ...
     */
    async emitFenceInline(channel, fence, opts = {}) {
     ...
     */
    async emitFenceAttachment(channel, fence, counter, opts = {}) {
        if (!this.adapter.sendFile) {
            await this.emitFenceInline(channel, fence, opts);
            return;
        }
        const filename = fence.ext === "Dockerfile"
            ? counter === 1
                ? "Dockerfile"
                : `Dockerfile.${counter}`
        ...
        try {
            ...
            if (buf.byteLength > MAX) {
                await this.adapter.sendMessage(channel, `_Code block too large to upload (${buf.byteLength} B, Discord 25 MB limit)._${opts.notice ? `\n\n${opts.notice}` : ""}`);
                return;
            }
            await this.adapter.sendFile(channel, {
                data: buf,
    ...
    // --- repo picker ---
    async sendRepoPicker(channel) {
        const dirs = this.listRepoDirs();
        if (!dirs) {
            await this.adapter.sendMessage(channel, `❌ REPOS_ROOT not found: \`${this.config.REPOS_ROOT}\``);
            return;
        }
        if (dirs.length === 0) {
            await this.adapter.sendMessage(channel, `⚠️ No repos under \`${this.config.REPOS_ROOT}\`. Use \`/seam repo <path>\`.`);
            return;
        }
        if (!this.adapter.sendChoicePicker) {
            ...
            const lines = dirs
                ...
                .join("\n");
            await this.adapter.sendMessage(channel, `🗂️ **Available repos**\n${this.renderer.codeBlock(lines)}\nUse \`/seam repo <name>\`.`);
            return;
        }
        ...
        if (!isWithinRoot(picked, this.config.REPOS_ROOT)) {
            await this.adapter.sendMessage(channel, `🛡️ Repo \`${picked}\` is outside REPOS_ROOT.`);
            return;
        }
        const record = this.router.ensureSessionRecord({
            platform: channel.platform,
        ...
        if (this.config.NEW_THREAD_WIZARD === "full") {
            await this.adapter.sendMessage(channel, `📌 Repo set to \`${this.repoDisplay(picked)}\`.`);
            ...
            await this.adapter.sendMessage(channel, `📌 Repo set to \`${this.repoDisplay(picked)}\`. Send a message to begin.`);
        }
    }
    listRepoDirs() {
     ...
     */
    async runSetupWizard(channel, record) {
        ...
        if (profiles.length > 1 && this.adapter.sendChoicePicker) {
            const picked = await this.adapter.sendChoicePicker(channel, {
                ...
                choices: profiles.map((p) => ({
                    value: p.id,
                    label: p.displayName,
                    description: p.id === currentRecord.agentId ? `${p.id} (current)` : p.id,
                })),
                authorizedUserIds: this.config.DISCORD_ALLOWED_USER_IDS,
                successPanel: (pickedChoice, username) => ({
                    color: 0x57f287,
            ...
            if (!picked) {
                ...
                await this.adapter.sendMessage(channel, `✅ Setup complete. Send a message to begin.`);
                return;
            }
        ...
        if (this.adapter.sendChoicePicker) {
            try {
                ...
                if (models.length > 1) {
                    ...
                    const picked = await this.adapter.sendChoicePicker(channel, {
                        ...
                        choices: models.slice(0, 25).map((m) => ({
                            value: m.modelId,
                            label: m.name ?? m.modelId,
                            description: m.modelId === current ? `${m.modelId} (current)` : m.modelId,
                        })),
                        authorizedUserIds: this.config.DISCORD_ALLOWED_USER_IDS,
                        successPanel: (pickedChoice, username) => ({
                            color: 0x57f287,
                ...
                this.logger.warn({ err }, "wizard: could not start runtime for model picker");
                await this.adapter.sendMessage(channel, `_Could not list models: ${err.message}. Use \`/seam model\` later._`);
            }
        }
        await this.adapter.sendMessage(channel, `✅ Setup complete. Send a message to begin.`);
    }
     ...
     * support it.
     */
    async renameThreadForSetup(channel, record) {
        if (!this.adapter.renameThread)
            return;
        if (!channel.parentId)
            return; // not a thread
        ...
        if (this.adapter.getThreadName) {
            ...
            if (current !== undefined && current !== "seam")
                return;
        }
     ...
     */
    async updateThreadAbbreviation(channel, oldAgentId, newAgentId) {
        ...
        try {
            ...
            const allAbbrs = this.router.listProfiles()
                .map((p) => p.threadAbbr)
                .filter((abbr) => typeof abbr === "string" && abbr.length > 0)
            ...
            if (replaced && newName !== currentName) {
                await this.adapter.renameThread(channel, newName);
                this.logger.info({ channelId: channel.id, oldName: currentName, newName }, "Updated thread name abbreviation on agent transition");
            }
        }
        catch (err) {
            this.logger.warn({ err, channelId: channel.id }, "Failed to update thread name abbreviation");
        }
    }
    // --- /seam preset … -------------------------------------------------------
    async cmdPreset(i) {
        ...
        switch (sub) {
            ...
            default:
                await i.reply({ content: `Unknown preset subcommand: ${sub}`, flags: MessageFlags.Ephemeral });
        }
    }
    presetSummaryLine(p) {
        const parts = [];
        ...
        if (p.model)
            parts.push(`Model: ${p.model}`);
        if (p.effort)
            parts.push(`Effort: ${p.effort}`);
        if (p.repoPath)
            parts.push(`Repo: ${this.repoDisplay(p.repoPath)}`);
        if (p.permission)
            parts.push(`Policy: ${p.permission}`);
        if (p.toolsAllow?.length)
            parts.push(`Allow: ${p.toolsAllow.join(", ")}`);
        if (p.toolsExclude?.length)
            parts.push(`Exclude: ${p.toolsExclude.join(", ")}`);
        if (p.instructions)
            parts.push("📝 Has instructions");
        const desc = p.description ? ` — ${p.description}` : "";
        const config = parts.length > 0 ? `\n   ${parts.join(" · ")}` : "";
        return `⚙️ **${p.name}**${desc}${config}`;
    }
    async cmdPresetList(i) {
        ...
        collector.on("collect", async (c) => {
            try {
                ...
                if (!preset) {
                    await c.reply({ content: "That preset no longer exists.", flags: MessageFlags.Ephemeral });
                    return;
                }
                if (action === "apply") {
                    const channel = this.channelRefFromInteraction(c);
                    if (!channel) {
                        await c.reply({ content: "Use inside a thread to apply a preset.", flags: MessageFlags.Ephemeral });
                        return;
                    }
                    const record = this.router.ensureSessionRecord({
                        platform: channel.platform,
                    ...
                    const summary = await this.applyPresetToSession(record, preset);
                    await c.reply({ content: `✅ Applied preset **${preset.name}**.\n${summary}`, flags: MessageFlags.Ephemeral });
                }
                else if (action === "edit") {
                    collector.stop("edit");
    ...
    buildPresetListMessage() {
        const presets = this.store.listPresets();
        const embed = new EmbedBuilder()
            .setTitle("🎛️ Presets")
            .setColor(PRESET_COLOR)
            .setDescription(presets.length
            ? presets.map((p) => this.presetSummaryLine(p)).join("\n\n")
            : "_No presets yet._");
        const components = [];
        for (const p of presets.slice(0, 5)) {
            components.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`pr:apply:${p.id}`).setLabel(`▶️ ${p.name}`.slice(0, 80)).setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`pr:edit:${p.id}`).setLabel("✏️ Edit").setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId(`pr:del:${p.id}`).setLabel("🗑️ Delete").setStyle(ButtonStyle.Danger)));
        }
        return { embeds: [embed], components };
    }
    async cmdPresetCreate(i) {
        await this.cmdPresetBuilder(i);
    }
    async cmdPresetEdit(i) {
    ...
    async cmdPresetBuilder(i, existing) {
        const profiles = this.router.listProfiles();
        const state = {
            name: existing?.name ?? "",
        ...
        const getModelsForAgent = (agentId) => {
            if (!agentId)
                return [];
        ...
        const render = () => {
            ...
            const repoDisplay = state.repoPath ? `\`${this.repoDisplay(state.repoPath)}\`` : "*(default)*";
            const permDisplay = state.permission ?? "*(default)*";
            const toolsDisplay = (() => {
                const parts = [];
                if (state.toolsAllow?.length)
                    parts.push(`Allow: ${state.toolsAllow.join(", ")}`);
                if (state.toolsExclude?.length)
                    parts.push(`Exclude: ${state.toolsExclude.join(", ")}`);
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(PRESET_COLOR)
                .setDescription("A preset is a reusable bundle of session settings. " +
                "When applied, it overrides only the fields it specifies — everything else keeps its default.")
                .addFields({ name: "🏷️ Name", value: state.name || "*(not set)*" }, { name: "📝 Description", value: state.description || "*(none)*" }, { name: "🤖 Agent", value: agentDisplay, inline: true }, { name: "🧠 Model", value: modelDisplay, inline: true }, { name: "⚡ Effort", value: effortDisplay, inline: true }, { name: "📂 Repo", value: repoDisplay, inline: true }, { name: "🔒 Permission", value: permDisplay, inline: true }, { name: "🔧 Tools", value: toolsDisplay }, { name: "📋 Instructions", value: instrDisplay });
            // Agent select
            const agentSelect = new StringSelectMenuBuilder()
                .setCustomId("preset:agent")
                .setPlaceholder("🤖 Agent")
                .addOptions({ label: "Default", value: "__default__", default: state.agentId === null }, ...profiles.map((p) => ({
                label: p.displayName,
                value: p.id,
                description: p.id,
                default: p.id === state.agentId,
            })));
            // Model select (populated from selected agent's staticModels)
            const models = getModelsForAgent(state.agentId);
            const modelSelect = new StringSelectMenuBuilder()
                .setCustomId("preset:model")
                .setPlaceholder("🧠 Model");
            if (models.length > 0) {
                modelSelect.addOptions({ label: "Default", value: "__default__", default: state.model === null }, ...models.map((m) => ({
                ...
                modelSelect.addOptions({ label: "Default (select an agent first for model list)", value: "__default__", default: true });
            }
            // Effort select
            const effortSelect = new StringSelectMenuBuilder()
                .setCustomId("preset:effort")
                .setPlaceholder("⚡ Effort")
                .addOptions({ label: "Default", value: "__default__", default: state.effort === null }, ...EFFORT_CHOICES.map((e) => ({
                label: e.label,
                value: e.value,
            ...
            // Buttons
            const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("preset:details").setLabel("✏️ Name & details").setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId("preset:tools").setLabel("🔧 Tools").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId("preset:instr").setLabel("📋 Instructions").setStyle(ButtonStyle.Secondary), new ButtonBuilder()
                .setCustomId("preset:save")
                .setLabel(existing ? "💾 Save" : "✅ Create")
                .setStyle(ButtonStyle.Success)
                .setDisabled(!state.name), new ButtonBuilder().setCustomId("preset:cancel").setLabel("Cancel").setStyle(ButtonStyle.Secondary));
            const rows = [
                new ActionRowBuilder().addComponents(agentSelect),
                new ActionRowBuilder().addComponents(modelSelect),
                new ActionRowBuilder().addComponents(effortSelect),
                buttons,
            ];
            return { embeds: [embed], components: rows };
        };
        ...
        collector.on("collect", async (c) => {
            try {
                if (c.isStringSelectMenu() && c.customId === "preset:agent") {
                    ...
                    // Reset model when agent changes (different model lists)
                    state.model = null;
                    await c.update(render());
                }
                    ...
                    // Open a modal for name, description, repo, and permission
                    const modal = new ModalBuilder()
                    ...
                    modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder()
                        ...
                        .setLabel("Permission: always / ask / deny (blank = default)")
                        .setStyle(TextInputStyle.Short)
                        .setMaxLength(10)
                    ...
                    await c.showModal(modal);
                    try {
                        const submit = await c.awaitModalSubmit({ time: 300_000, filter: (m) => m.customId === "preset:details-modal" });
                        ...
                        const permVal = submit.fields.getTextInputValue("permission").trim().toLowerCase();
                        state.permission = (permVal === "always" || permVal === "ask" || permVal === "deny") ? permVal : null;
                    ...
                    await c.showModal(modal);
                    try {
                        const submit = await c.awaitModalSubmit({ time: 300_000, filter: (m) => m.customId === "preset:tools-modal" });
                    ...
                    modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder()
                        .setCustomId("instr")
                        .setLabel("Instructions (appended to system prompt)")
                        .setStyle(TextInputStyle.Paragraph)
                        .setMaxLength(4000)
                    ...
                    await c.showModal(modal);
                    try {
                        const submit = await c.awaitModalSubmit({ time: 300_000, filter: (m) => m.customId === "preset:instr-modal" });
                    ...
                    // Check for name collision (only on create, or if name changed during edit)
                    if (!existing || existing.name.toLowerCase() !== state.name.toLowerCase()) {
                        const collision = this.store.getPresetByName(state.name);
                        if (collision) {
                            await c.reply({ content: `A preset named \`${state.name}\` already exists.`, flags: MessageFlags.Ephemeral });
                            return;
                        }
                    }
                    const now = new Date().toISOString();
    ...
    async cmdPresetApply(i) {
        ...
        const channel = this.channelRefFromInteraction(i);
        if (!channel) {
            await i.reply({ content: "Use `/seam preset apply` inside a thread.", flags: MessageFlags.Ephemeral });
            return;
        }
        const record = this.router.ensureSessionRecord({
            platform: channel.platform,
    ...
    /**
     * Apply a preset's settings to a session record. Returns a human-readable
     * summary of what was changed. Handles agent switching (which resets the
     * session) as well as config-level overrides.
     */
    async applyPresetToSession(record, preset) {
        const changes = [];
        // Agent change must happen first — it resets config and invalidates runtime
        if (preset.agentId && preset.agentId !== record.agentId) {
            ...
            if (profile) {
                ...
                // Refresh the record after agent change
                record = this.store.get(record.id) ?? record;
                changes.push(`Agent → \`${preset.agentId}\``);
            }
        }
        // Now apply config-level overrides
        const cfg = this.store.readConfig(record);
        if (preset.model) {
            cfg.model = preset.model;
            cfg.lastContextUsage = undefined; // model changed
        ...
        // Invalidate runtime so next message picks up all changes
        await this.router.invalidate(record.id);
        return changes.length > 0
            ? changes.map((c) => `• ${c}`).join("\n")
            : "_(no overrides — all fields use defaults)_";
    }
    async cmdPresetShow(i) {
        const name = i.options.getString("name", true);
        const preset = this.store.getPresetByName(name);
        if (!preset) {
            await i.reply({ content: `No preset named \`${name}\`.`, flags: MessageFlags.Ephemeral });
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: "🤖 Agent", value: preset.agentId ? `\`${preset.agentId}\`` : "*(default)*", inline: true }, { name: "🧠 Model", value: preset.model ? `\`${preset.model}\`` : "*(default)*", inline: true }, { name: "⚡ Effort", value: preset.effort ?? "*(default)*", inline: true }, { name: "📂 Repo", value: preset.repoPath ? `\`${this.repoDisplay(preset.repoPath)}\`` : "*(default)*", inline: true }, { name: "🔒 Permission", value: preset.permission ?? "*(default)*", inline: true }, { name: "🔧 Tools allow", value: preset.toolsAllow?.join(", ") || "*(all)*" }, { name: "🔧 Tools exclude", value: preset.toolsExclude?.join(", ") || "*(none)*" }, { name: "📋 Instructions", value: preset.instructions ? "```\n" + preset.instructions.slice(0, 1000) + "\n```" : "*(none)*" })
            .setFooter({ text: `ID: ${preset.id} · Created by <@${preset.createdBy}> · ${preset.createdUtc}` });
        await i.reply({ embeds: [embed], flags: MessageFlags.Ephemeral });
    }
    async cmdPresetDelete(i) {
        const name = i.options.getString("name", true);
        const preset = this.store.getPresetByName(name);
        if (!preset) {
            await i.reply({ content: `No preset named \`${name}\`.`, flags: MessageFlags.Ephemeral });
    ...
    // --- helpers ---
    channelRefFromInteraction(i) {
        if (!i.channelId)
            return undefined;
        const ch = i.channel;
        const parentId = ch && "parentId" in ch && typeof ch.parentId === "string"
            ? ch.parentId
            : undefined;
        return {
            platform: PLATFORM,
...
function fitTranscriptToWindow(transcript, templateOverhead, modelWindowTokens) {
    const maxChars = Math.floor(modelWindowTokens * 4 * 0.8);
    const targetLen = Math.max(0, maxChars - templateOverhead);
    if (transcript.length <= targetLen)
        return transcript;
...
function formatAgyUsage(d) {
    ...
    const modelsWithQuota = d.models.filter((m) => typeof m.remainingFraction === "number" || m.resetTime);
...
function formatCopilotUsage(d) {
    ...
    if (quotas.length > 0) {
        ...
        if (d.quotaResetAt)
            lines.push(`Resets ${formatResetTime(d.quotaResetAt)}`);
    }
    return lines.join("\n");
}
function formatClaudeUsage(d) {
    const lines = [];
    ...
    if (d.extraUsage && d.extraUsage.enabled) {
        ...
        lines.push("", "**Usage credits**", usageLine(d.extraUsage.utilization, `${dollars(d.extraUsage.used)} / ${dollars(d.extraUsage.limit)}`));
    }
    return lines.join("\n");
}
...
function renderImagePicker(state, opts = {}) {
    ...
    if (status === "generating")
        title = "🎨 Generating…";
    if (status === "done")
        title = `✅ Generated in ${opts.elapsedSec ?? "?"}s`;
    if (status === "error")
        title = "❌ Generation failed";
    const promptPreview = state.prompt.length > 1000
    ...
    embed.addFields({ name: "Model", value: `${model.displayName}`, inline: true }, { name: "Aspect", value: state.aspectRatio, inline: true }, { name: "Resolution", value: state.resolution, inline: true }, { name: "Count", value: `${state.count}`, inline: true });
    ...
    // Model select
    const visibleModels = IMAGE_MODELS.filter((m) => m.provider !== "bfl" || state.bflKeyAvailable);
    const modelSelect = new StringSelectMenuBuilder()
        .setCustomId("img:model")
        .setPlaceholder("Pick model")
        .addOptions(visibleModels.map((m) => ({
        label: m.displayName,
    ...
    const aspectRow = model.aspectRatios.length <= 5
        ? new ActionRowBuilder().addComponents(...model.aspectRatios.map((ar) => new ButtonBuilder()
        ...
        : new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
            ...
            .addOptions(model.aspectRatios.map((ar) => ({
            label: ar,
            value: ar,
            default: ar === state.aspectRatio,
        }))));
    // Resolution row (1K / 2K / 4K — per-model)
    const resolutionRow = new ActionRowBuilder().addComponents(...model.resolutions.map((r) => new ButtonBuilder()
    ...
    const countChoices = [];
    for (let n = 1; n <= Math.min(model.maxCount, 4); n++)
        countChoices.push(n);
    const countRow = new ActionRowBuilder().addComponents(...countChoices.map((n) => new ButtonBuilder()
    ...
    actionButtons.push(new ButtonBuilder().setCustomId("img:edit").setLabel("✍️ Edit").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId("img:generate").setLabel("✨ Generate").setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId("img:cancel").setLabel("✖️ Cancel").setStyle(ButtonStyle.Danger));
    return {
        embeds: [embed],
        components: [
            new ActionRowBuilder().addComponents(modelSelect),

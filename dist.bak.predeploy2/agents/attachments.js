 ...
 */
export async function mapAttachmentsToBlocks(attachments, opts = {}) {
    ...
    for (const a of limited) {
        ...
        try {
            ...
            opts.logger?.warn({ err, filename: a.filename, url: a.url }, "attachment download failed; falling back to resource_link");
            blocks.push(toResourceLink(a));
        }
    }
    return { blocks, rejected };
}
function toResourceLink(a) {
    return {
        type: "resource_link",
        name: a.filename,
        uri: `attachment://${a.filename}`,
...
function isTextLikeMime(mime, filename) {
    if (mime) {
        if (TEXT_MIME_PREFIXES.some((p) => mime.startsWith(p)))
            return true;
        if (TEXT_MIME_EXACT.has(mime))
            return true;
        if (TEXT_MIME_SUFFIXES.some((s) => mime.endsWith(s)))
            return true;
    }
...
export function isModelInlineableAttachment(mime, filename) {
    const m = (mime || "").toLowerCase();
    if (MODEL_IMAGE_MIMES.has(m))
        return true;
...
export function isInlineableForAgent(mime, filename, agentHasVision) {
    if (!isModelInlineableAttachment(mime, filename))
        return false;
    const isImage = (mime || "").toLowerCase().startsWith("image/");
    return !(isImage && agentHasVision === false);
}
async function downloadBase64(url, fetchFn) {
    const buf = await downloadBytes(url, fetchFn);
    return Buffer.from(buf).toString("base64");
}
async function downloadText(url, fetchFn) {
    const buf = await downloadBytes(url, fetchFn);

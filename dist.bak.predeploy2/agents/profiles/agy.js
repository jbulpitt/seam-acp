...
import { discoverAgyLs, subscribeToAgyStream, waitForAgyConversationId, } from "../agy-stream.js";
...
async function newSpawnLogPath() {
 ...
 */
function conversationMaxStepIndex(cascadeId) {
    const dbPath = path.join(CONVERSATION_DIR, `${cascadeId}.db`);
    try {
        const db = new Database(dbPath, { readonly: true, fileMustExist: true });
        try {
            const row = db.prepare("SELECT MAX(idx) AS m FROM steps").get();
        ...
        try {
            ...
            outer: while (true) {
                    ...
                    for await (const update of subscribeToAgyStream({
                        ...
                        if ((mainIdle || fully) && hasBeenActive) {
                            if (!fully) {
                                ...
                                if (process.env.AGY_PROFILE_DEBUG) {
                                    // eslint-disable-next-line no-console
                                    console.error(`[agy] main trajectory idle but NOT fullyIdle — continuing to wait`, { status: update.status, fullyIdle: update.fullyIdle,
                ...
                if (cancelAbort.signal.aborted)
                    break;
                if (procExited)
                    break; // agy itself exited — no point retrying
                const remaining = staleIdleDeadline - Date.now();
                if (remaining <= 0) {
                    if (process.env.AGY_PROFILE_DEBUG) {
                        // eslint-disable-next-line no-console
                        console.error(`[agy] stale-idle retry deadline exceeded after ${staleIdleRetryCount} retries`);
                    }
                    break;
                }
                ...
                if (process.env.AGY_PROFILE_DEBUG) {
                    // eslint-disable-next-line no-console
                    console.error(`[agy] stream ended with no activity (stale idle) — retry #${staleIdleRetryCount} in ${STALE_IDLE_RETRY_DELAY_MS}ms`);
                }
                await new Promise((r) => setTimeout(r, STALE_IDLE_RETRY_DELAY_MS));
            }
            ...
            if (cancelAbort.signal.aborted && !runRef.userCancelled) {
                ...
                // eslint-disable-next-line no-console
                console.error(`[agy] process exited during prompt (code=${exitCode}, signal=${proc.signalCode ?? "none"})` +
...
function buildAgyConfigOptions(catalog, currentModelId) {
    return [
        {
            id: "model",
            name: "Model",

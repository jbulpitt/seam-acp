 ...
 */
async function resolveProjectDir(claudeDir, cwd) {
...
export function makeClaudeProfile(opts) {
    ...
    return {
        ...
        spawn(modelOverride, _effortOverride) {
        ...
        sessionManager: {
                ...
                for (let i = lines.length - 1; i >= 0; i--) {
                    try {
                        ...
                        if (newerThanMs !== undefined && entry.timestamp) {
                            ...
                            if (!isNaN(entryMs) && entryMs < newerThanMs)
                                continue;
                        }
            ...
            async getHistoryPath(cwd, sessionId) {
                const dir = configDir ?? path.join(process.env.HOME ?? "", ".claude");
                const projectDir = await resolveProjectDir(dir, cwd);
                const file = path.join(projectDir, `${sessionId}.jsonl`);
                try {
            ...
            async repairSession(cwd, sessionId) {
                const dir = configDir ?? path.join(process.env.HOME ?? "", ".claude");
                const projectDir = await resolveProjectDir(dir, cwd);
                const file = path.join(projectDir, `${sessionId}.jsonl`);
                const content = await fsp.readFile(file, "utf8");
 ...
 */
export async function fetchClaudeUsage(configDir) {
    ...
    for (const file of candidates) {
        try {
            ...
            result.login =
                pickLogin(parsed) ??
                    (oauth ? pickStringField(oauth, ["email", "login"]) : undefined) ??
                    null;
...
function pickStringField(obj, fields) {
...
function claudeContextWindowFamily(id) {
    if (/opus-(?:4[.-](?:[7-9]|\d{2,})|[5-9]|\d{2,})\b/.test(id))
        return 1_000_000;
    if (/(?:sonnet|fable|mythos)-(?:[5-9]|\d{2,})\b/.test(id))
        return 1_000_000;
    return 200_000;
}
...
export function isForwardableFullModelId(modelId) {
    if (!modelId)
        return false;
...
export function getClaudeContextWindow(modelId) {
    ...
    if (id === "default")
        return 1_000_000;
    if (/\b1m\b/.test(id))
        return 1_000_000; // tolerate any residual legacy suffix
...
function withClaudeContextLimits(models) {
    return models.map((m) => ({
        ...m,
...
function isAdaptiveThinkingModel(modelId) {
    ...
    // left alone.
    return (/opus-(?:4[.-](?:[6-9]|\d{2,})|[5-9]|\d{2,})\b/.test(m) ||

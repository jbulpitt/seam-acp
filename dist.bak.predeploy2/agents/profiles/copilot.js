    ...
    return {
        ...
        sessionManager: {
                    ...
                    try {
                        ...
                        try {
                            ...
                            try {
                                ...
                                const rows = seamDb.prepare("SELECT acp_session_id FROM sessions WHERE repo_path = ?").all(cwd);
                        ...
                        for (const sess of sessions) {
                            ...
                            const turns = db.prepare("SELECT * FROM turns WHERE session_id = ? ORDER BY turn_index ASC").all(sessionId);
                            const allMessages = [];
            ...
            async cloneSession(cwd, oldSessionId, newSessionId) {
                ...
                const db = new Database(dbPath);
                try {
                    const sessionRow = db.prepare("SELECT * FROM sessions WHERE id = ?").get(oldSessionId);
                    if (sessionRow) {
                        const nowIso = new Date().toISOString();
                        db.prepare(`
              INSERT INTO sessions (id, cwd, repository, host_type, branch, summary, created_at, updated_at)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `).run(newSessionId, cwd, sessionRow.repository, sessionRow.host_type, sessionRow.branch, sessionRow.summary, nowIso, nowIso);
                    }
                    const turns = db.prepare("SELECT * FROM turns WHERE session_id = ? ORDER BY turn_index ASC").all(oldSessionId);
                    const insertTurn = db.prepare(`
            INSERT INTO turns (session_id, turn_index, user_message, assistant_response, timestamp)
            VALUES (?, ?, ?, ?, ?)
          `);
                    for (const turn of turns) {
                        insertTurn.run(newSessionId, turn.turn_index, turn.user_message, turn.assistant_response, turn.timestamp);
                    }
 ...
 */
function readCopilotTokenSync(configDir) {
 ...
 */
async function readCopilotIdentity(configDir) {
    const dir = configDir ?? path.join(process.env.HOME ?? "", ".copilot");
 ...
 */
export async function fetchCopilotUsage(configDir) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            result.login = body.login ?? result.login;
            result.plan = body.copilot_plan ?? null;
            result.quotaResetAt = body.quota_reset_date_utc ?? null;
            const orgs = body.organization_list;
            if (orgs && orgs.length > 0)
                result.org = orgs[0]?.name ?? orgs[0]?.login ?? null;
 ...
 */
function buildCopilotMcpConfigJson(servers) {
    if (servers.length === 0)
        return undefined;
    const map = {};

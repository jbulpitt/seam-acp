...
const Schema = z.object({
    ...
    COPILOT_PROFILES: z
        ...
        for (const entry of v.split(",").map((s) => s.trim()).filter(Boolean)) {
            const idx = entry.indexOf(":");
            if (idx <= 0 || idx === entry.length - 1) {
                throw new Error(`COPILOT_PROFILES entry must be 'id:/abs/path' (got '${entry}')`);
            }
            const id = entry.slice(0, idx).trim();
            const dir = path.resolve(entry.slice(idx + 1).trim());
            if (!/^[a-z0-9][a-z0-9-]*$/i.test(id)) {
                throw new Error(`COPILOT_PROFILES id '${id}' must be alphanumeric (dashes allowed)`);
            }
    ...
    CLAUDE_PROFILES: z
        ...
        for (const entry of v.split(",").map((s) => s.trim()).filter(Boolean)) {
            const idx = entry.indexOf(":");
            if (idx <= 0 || idx === entry.length - 1) {
                throw new Error(`CLAUDE_PROFILES entry must be 'id:/abs/path' (got '${entry}')`);
            }
            const id = entry.slice(0, idx).trim();
            const dir = path.resolve(entry.slice(idx + 1).trim());
            if (!/^[a-z0-9][a-z0-9-]*$/i.test(id)) {
                throw new Error(`CLAUDE_PROFILES id '${id}' must be alphanumeric (dashes allowed)`);
            }
            out.push({ id, configDir: dir });
        }
        return out;
    ...
    REMOTE_COPILOT_PROFILES: z
        ...
        for (const entry of v.split(",").map((s) => s.trim()).filter(Boolean)) {
            ...
            if (first <= 0) {
                throw new Error(`REMOTE_COPILOT_PROFILES entry must be 'id:port:token' or 'id:wsUrl:token' (got '${entry}')`);
            }
            const id = entry.slice(0, first).trim();
            if (!/^[a-z0-9][a-z0-9-]*$/i.test(id)) {
                throw new Error(`REMOTE_COPILOT_PROFILES id '${id}' must be alphanumeric (dashes allowed)`);
            }
            ...
            if (rest.startsWith("ws://") || rest.startsWith("wss://")) {
                ...
                if (lastColon <= "wss://".length || lastColon === rest.length - 1) {
                    throw new Error(`REMOTE_COPILOT_PROFILES client entry must be 'id:wsUrl:token' (got '${entry}')`);
                }
                ...
                if (second <= 0 || second === rest.length - 1) {
                    throw new Error(`REMOTE_COPILOT_PROFILES server entry must be 'id:port:token' (got '${entry}')`);
                }
                ...
                if (!Number.isInteger(wsPort) || wsPort < 1 || wsPort > 65535) {
                    throw new Error(`REMOTE_COPILOT_PROFILES port '${portStr}' must be a valid port number`);
                }
...
const PresetFieldSchema = (value) => z.object({ value });
 ...
 */
export function resolveChannelPreset(config, parentId, threadId) {
    ...
    const riders = [chan?.rider?.value, thread?.rider?.value].filter((v) => !!v);
    return {
...
export function isChannelLocked(config, parentId) {
    if (!parentId)
        return false;
...
export function loadConfig() {
    ...
    if (!fs.existsSync(reposRoot) || !fs.statSync(reposRoot).isDirectory()) {
        throw new Error(`REPOS_ROOT does not exist or is not a directory: ${reposRoot}\n` +
...
function loadChannelPresets(file) {
    const channelPresets = new Map();
    const threadPresets = new Map();
    if (!file)
    ...
    try {
        ...
        throw new Error(`CHANNEL_PRESETS_FILE could not be read: ${abs} (${err.message})`);
    }
    let parsed;
    try {
        ...
        throw new Error(`CHANNEL_PRESETS_FILE is not valid JSON: ${abs} (${err.message})`);
    }
    ...
    for (const [channelId, preset] of Object.entries(result.data.channels)) {
        ...
        if (preset.cwd)
            normalized.cwd = { value: path.resolve(preset.cwd.value) };

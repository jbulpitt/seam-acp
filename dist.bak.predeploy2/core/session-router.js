...
export class SessionRouter {
    ...
    askUser;
    runtimes = new Map();
    creationLocks = new Map();
    lastStartFailure = new Map();
    startFailureCooldownMs = 30_000;
    constructor(opts) {
    ...
    ensureSessionRecord(opts) {
        ...
        const preset = resolveChannelPreset({ channelPresets: this.channelPresets, threadPresets: this.threadPresets }, opts.parentRef ?? undefined, opts.channelRef);
        const cfg = defaultSessionConfig(preset.model?.value ?? this.defaultModel, this.defaultPermissionMode);
     ...
     */
    async getOrStartRuntime(record) {
        ...
        if (lastFail && Date.now() - lastFail < this.startFailureCooldownMs) {
            const wait = Math.ceil((this.startFailureCooldownMs - (Date.now() - lastFail)) / 1000);
            throw new Error(`Agent recently failed to start; waiting ${wait}s before retry.`);
        }
        const promise = this.startRuntime(record).then((rt) => {
    ...
    async invalidate(sessionId, opts) {
        ...
        if (opts?.clearAcpSession) {
            ...
            if (record?.acpSessionId) {
                ...
                if (record.agentId?.startsWith("agy")) {
                    this.logger.info({ sessionId }, "preserving agy acp/cascade id across invalidate (durable across agy -p spawns)");
                }
                else {
    ...
    async abortTurn(sessionId, opts) {
        const rt = this.runtimes.get(sessionId);
        if (!rt)
            return "idle";
    ...
    async killAll(opts) {
        ...
        for (const id of ids) {
            await this.invalidate(id, { clearAcpSession: false }).catch((err) => this.logger.warn({ err, id }, "killAll: invalidate failed"));
        }
        return ids.length;
    }
    ...
    async disposeAll() {
        ...
        this.runtimes.clear();
        await Promise.all(all.map((rt) => rt.dispose().catch((err) => {
    ...
    async startRuntime(record) {
        ...
        const preset = resolveChannelPreset({ channelPresets: this.channelPresets, threadPresets: this.threadPresets }, record.parentRef ?? undefined, record.channelRef);
        const agentId = preset.agent?.value ?? record.agentId;
        const profile = this.profileById.get(agentId);
        if (!profile) {
            throw new Error(`Unknown agent profile "${agentId}" for session ${record.id}`);
        }
        ...
        // erroring.
        const presetEffortUsable = preset.effort?.value &&
            profile.effort &&
        ...
        const runtime = new AgentRuntime({
            ...
            permissionPolicy: async (req) => {
                ...
                if (mode === "always") {
                    const opt = req.options.find((o) => o.kind?.startsWith("allow_")) ??
                        req.options[0];
                    if (opt) {
                        return {
        ...
        if (record.acpSessionId) {
            ...
            for (let attempt = 1; attempt <= RESUME_ATTEMPTS; attempt++) {
                try {
                    ...
                    this.logger.debug({ sessionId: record.id, acpSessionId: record.acpSessionId, attempt }, "resumed acp session");
                    return runtime;
                }
                catch (err) {
                    const lastAttempt = attempt === RESUME_ATTEMPTS;
                    this.logger.warn({ err, sessionId: record.id, attempt, lastAttempt }, lastAttempt

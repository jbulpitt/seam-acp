...
async function main() {
    ...
    const extraCopilots = config.COPILOT_PROFILES.map((p) => makeCopilotProfile({
        id: `copilot-${p.id}`,
    ...
    const extraClaudes = config.CLAUDE_PROFILES.map((p) => makeClaudeProfile({
        id: `claude-${p.id}`,
    ...
    if (config.OPENCODE_ENABLED && config.OPENCODE_LMSTUDIO_URL) {
        const discovered = await fetchLmStudioModels(config.OPENCODE_LMSTUDIO_URL, config.OPENCODE_LMSTUDIO_API_KEY || undefined, config.OPENCODE_MODEL_PREFIX).catch((err) => {
        ...
        if (discovered.length > 0) {
            const opencodeConfigPath = config.OPENCODE_CONFIG_PATH ||
                path.join(process.env.XDG_CONFIG_HOME || path.join(process.env.HOME ?? "", ".config"), "opencode", "opencode.json");
    ...
    let notifyBridgeConnect = () => { };
    const remoteCopilots = config.REMOTE_COPILOT_PROFILES.map((p) => p.mode === "server"
    ...
    notifyBridgeConnect = (id) => void orchestrator.postNotification(`🟢 Remote bridge connected: ${id}`);

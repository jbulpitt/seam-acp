import { Client, GatewayIntentBits, Partials, Events, ChannelType, REST, Routes, MessageFlags, MessageType, ActionRowBuilder, ButtonBuilder, ButtonStyle, ComponentType, EmbedBuilder, StringSelectMenuBuilder, } from "discord.js";
...
export class DiscordAdapter {
     ...
     */
    async sendChoicePicker(channel, opts) {
        const ch = await this.fetchSendableChannel(channel.id);
        ...
        const components = [];
        if (useButtons) {
            const buttons = choices.map((c, idx) => new ButtonBuilder()
                .setCustomId(`${customId}:${idx}`)
                .setLabel(c.label.slice(0, 80))
                .setStyle(ButtonStyle.Secondary));
            for (let i = 0; i < buttons.length; i += BUTTONS_PER_ROW) {
                components.push(new ActionRowBuilder().addComponents(buttons.slice(i, i + BUTTONS_PER_ROW)));
            }
        }
        else {
        ...
        try {
            ...
            if (interaction.componentType === ComponentType.Button) {
                pickedIdx = Number.parseInt(interaction.customId.split(":").pop() ?? "", 10);
            }
            ...
            if (opts.successPanel) {
                const successPanel = opts.successPanel({ value: chosen.value, label: chosen.label }, interaction.user.username);
    ...
    async createThread(parent, name) {
        let ch = await this.client.channels.fetch(parent.id);
        if (!ch)
            throw new Error(`Channel ${parent.id} not found`);
        ...
        if (ch.type !== ChannelType.GuildText &&
            ...
            throw new Error(`Channel ${ch.id} (type ${ch.type}) does not support threads`);
        }
    ...
    async fetchThreadMessages(channel) {
        const ch = await this.fetchSendableChannel(channel.id);
        if (!ch.isThread())
        ...
        while (true) {
            ...
            for (const msg of chunk.values()) {
                if (msg.type !== MessageType.Default && msg.type !== MessageType.Reply)
                    continue;
                if (!msg.content?.trim() && msg.attachments.size === 0)
                    continue;
                ...
                if (msg.author.bot && msg.embeds.length > 0 && !msg.content?.trim())
                    continue;
    ...
    async getThreadLiveState(channel) {
        try {
            const ch = await this.client.channels.fetch(channel.id);
            if (!ch)
            ...
            if (ch.isTextBased())
                return { locked: false, archived: false }; // plain channel — always postable
    ...
    async fetchThreadMessagesTimed(channel, opts) {
        const ch = await this.fetchSendableChannel(channel.id);
        if (!ch.isThread())
            throw new Error("Channel is not a thread.");
    ...
    async sendPanel(channel, panel) {
        const ch = await this.fetchSendableChannel(channel.id);
        const embed = DiscordAdapter.buildEmbed(panel);
        const sent = await ch.send({ embeds: [embed] });
        return { channel, id: sent.id };
    }
    async editPanel(message, panel) {
        const ch = await this.fetchSendableChannel(message.channel.id);
        const msg = await ch.messages.fetch(message.id);
        const embed = DiscordAdapter.buildEmbed(panel);
        await msg.edit({ content: "", embeds: [embed] });
    }
    static buildEmbed(panel) {
    ...
    wire() {
        ...
        this.client.on(Events.InteractionCreate, (interaction) => {
            if (!interaction.isChatInputCommand())
                return;
            if (interaction.commandName !== "seam")
                return;
    ...
    async handleMessage(msg) {
        ...
        if (msg.type !== MessageType.Default && msg.type !== MessageType.Reply)
            return;
        if (!this.config.DISCORD_ALLOWED_USER_IDS.has(msg.author.id))
            return;
        ...
        if (allowedChannels && (!parentId || !allowedChannels.has(parentId)))
            return;
    ...
    async pushAvatar() {
        const avatarPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../../assets/seam-acp-avatar.png");
    ...
    async pushBanner() {
        const bannerPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../../assets/seam-acp-banner.png");
     ...
     * answer.
     */
    async requestApproval(channel, req, opts = {}) {
        const timeoutMs = opts.timeoutMs ?? 5 * 60 * 1000;
        const ch = await this.fetchSendableChannel(channel.id);
        ...
        if (tool?.kind)
            embed.addFields({ name: "Tool kind", value: tool.kind, inline: true });
        if (tool?.toolCallId)
        ...
        const buttons = req.options.slice(0, 5).map((opt, idx) => new ButtonBuilder()
        ...
        if (guildIds.length > 0) {
            ...
            for (const guildId of guildIds) {
                ...
                try {
                    ...
                    this.logger.warn({ err, guildId }, "failed to register guild slash commands — is the bot a member of this guild? skipping; other guilds unaffected");
                }
            }
        }
        else {
...
function buttonStyleForKind(kind) {
    switch (kind) {
        case "allow_always":
...
function buttonEmojiForKind(kind) {
    switch (kind) {
        case "allow_always":
            return "✅";
        case "allow_once":

...
export function buildSeamCommand() {
    ...
    cmd.addSubcommand((sub) => sub
        ...
        .addStringOption((o) => o.setName("id").setDescription("Model id").setRequired(false)));
    cmd.addSubcommand((sub) => sub
        .setName("mode")
        .setDescription("Set the agent operational mode")
        .addStringOption((o) => o.setName("id").setDescription("Mode id").setRequired(true)));
    cmd.addSubcommand((sub) => sub
        .setName("effort")
        ...
        .setRequired(false)
        .addChoices({ name: "low", value: "low" }, { name: "medium", value: "medium" }, { name: "high", value: "high" }, { name: "xhigh", value: "xhigh" }, { name: "max", value: "max" })));
    cmd.addSubcommand((sub) => sub.setName("abort").setDescription("Stop the current turn — cancel, then force-kill if it's hung"));
    cmd.addSubcommand((sub) => sub.setName("cancel").setDescription("Gracefully cancel the current turn (cleaner; use abort if it ignores it)"));
    cmd.addSubcommand((sub) => sub.setName("kill").setDescription("Force-kill ALL active agent sessions, including this thread (each resumes on next message)"));
    cmd.addSubcommand((sub) => sub
        .setName("image")
    ...
    cmd.addSubcommand((sub) => sub
        .setName("reset")
        .setDescription("End the current ACP session for this thread; next message starts fresh"));
    cmd.addSubcommand((sub) => sub
        .setName("tools")
        ...
        .setRequired(true)
        .addChoices({ name: "allow", value: "allow" }, { name: "exclude", value: "exclude" }))
        .addStringOption((o) => o
        .setName("list")
        .setDescription("Comma-separated tool names (empty = clear)")
        .setRequired(false)));
    cmd.addSubcommand((sub) => sub.setName("config").setDescription("Show current session config"));
    cmd.addSubcommand((sub) => sub
        .setName("config-set")
        .setDescription("Replace session config with a JSON blob")
        .addStringOption((o) => o.setName("json").setDescription("Config JSON").setRequired(true)));
    cmd.addSubcommand((sub) => sub.setName("sessions").setDescription("List recent sessions"));
    cmd.addSubcommand((sub) => sub.setName("repos").setDescription("List repos under REPOS_ROOT"));
    cmd.addSubcommand((sub) => sub
        .setName("init")
    ...
    cmd.addSubcommand((sub) => sub
        ...
        .setRequired(true)
        .addChoices({ name: "always (auto-approve everything)", value: "always" }, { name: "ask (prompt me on Discord)", value: "ask" }, { name: "deny (auto-deny everything)", value: "deny" })));
    cmd.addSubcommand((sub) => sub
        .setName("agent")
        .setDescription("Get or set the agent for this thread (resets the session when changed)")
        .addStringOption((o) => o
        .setName("id")
    ...
    cmd.addSubcommand((sub) => sub
        ...
        .addStringOption((o) => o
        .setName("path")
        .setDescription("Absolute path, or path relative to an allowed root (REPOS_ROOT / ATTACH_ROOTS)")
        .setRequired(true)));
    cmd.addSubcommand((sub) => sub.setName("whoami").setDescription("Show which account this thread's agent is signed in as"));
    cmd.addSubcommand((sub) => sub.setName("usage").setDescription("Show usage / credits for this thread's agent (agy only)"));
    cmd.addSubcommand((sub) => sub.setName("avatar").setDescription("Push the bot avatar and banner to Discord (force re-upload)"));
    cmd.addSubcommand((sub) => sub.setName("help").setDescription("Show help"));
    cmd.addSubcommandGroup((g) => g
        ...
        .addAttachmentOption((o) => o.setName("file").setDescription("Optional reference file (re-sent on every run)").setRequired(false))
        .addAttachmentOption((o) => o.setName("file2").setDescription("Optional reference file").setRequired(false))
        .addAttachmentOption((o) => o.setName("file3").setDescription("Optional reference file").setRequired(false)))
        .addSubcommand((sub) => sub.setName("list").setDescription("List this thread's scheduled prompts"))
        .addSubcommand((sub) => sub
        .setName("remove")
    ...
    cmd.addSubcommandGroup((g) => g
        ...
        .addSubcommand((sub) => sub.setName("create").setDescription("Create a new preset (opens a builder card)"))
        .addSubcommand((sub) => sub
        .setName("apply")
...
export function getSubcommand(i) {
    return i.options.getSubcommand(true);
}

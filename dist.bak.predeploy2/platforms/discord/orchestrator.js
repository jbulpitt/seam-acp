...
import { readRichHistory, renderHistory } from "../../core/compaction/source-reader.js";
...
import { isSessionRecord, } from "../../core/inject-turn.js";
...
export class Orchestrator {
    ...
    async handleIncomingMessageInner(msg) {
        ...
        const modelContextFloor = turnProfile?.staticModels?.find((m) => m.modelId === activeModel)?.contextLimit
        ...
        const HARD_MAX = 1800;
        const SOFT_MIN = 800;
        const drainBufferInner = async (force, allowUnsafeCut = false) => {
            while (textBuffer) {
        ...
        const drainBuffer = (force, allowUnsafeCut = false) => flushQueue.run(() => drainBufferInner(force, allowUnsafeCut));
        ...
        try {
            ...
            const eventHandler = async (event) => {
                ...
                if (turnFinalized &&
                    ...
                    if (prev)
                        clearTimeout(prev);
                    this.bgSettleTimers.set(record.id, setTimeout(() => {
                        this.bgSettleTimers.delete(record.id);
                ...
                switch (event.kind) {
                    case "agent-text": {
                        ...
                        const isNewMessage = record.agentId.startsWith("copilot") &&
            ...
            const { riders } = resolveChannelPreset(this.config, record.parentRef ?? undefined, record.channelRef);
            ...
            if (activeProfile?.restrictDiscordAccess &&
                ...
                const { inline, hint } = await this.partitionAndStageAttachments(msg.attachments, agentHasVision);
                if (hint)
                    promptText = promptText ? `${promptText}${hint}` : hint.trimStart();
            ...
            if (result !== "timeout" && !result.cancelled) {
                ...
                if (usageReader) {
                    try {
                        ...
                        const modelEntry = profile?.staticModels?.find((m) => m.modelId === selectedModel) ?? profile?.staticModels?.find((m) => m.modelId === profile.defaultModel);
    ...
    static LOCK_EXEMPT_SUBCOMMANDS = new Set(["abort", "cancel"]);
    async handleSlashInteraction(interaction) {
        ...
        if (isChannelLocked(this.config, scopeChannelId) &&
     ...
     */
    async injectTurn(target, prompt, opts) {
        ...
        const outputTo = opts.outputTo ?? (target && !isSessionRecord(target) ? target : undefined);
        let text = "";
        ...
        const runPrompt = async (rt) => opts.timeoutMs === undefined
    ...
    async runPremiumCompactionForDiscord(args) {
        const { profile, manager, sessionId, cwd, channel, onProgress } = args;
        const log = (m) => { onProgress?.(m); this.logger.debug({ compaction: `discord-${sessionId}` }, m); };
        if (!channel) {
     ...
     */
    async dispatchInjectTurn(spec) {
        const target = { platform: PLATFORM, id: spec.target };
        const record = this.router.ensureSessionRecord({
            platform: PLATFORM,
        ...
        if (spec.session === "live" && (spec.model || spec.effort)) {
            ...
            this.logger.warn({ dispatch: spec.id, target: spec.target, model: spec.model, effort: spec.effort }, "dispatch: model/effort ignored for session=live (thread config wins); use session=isolated to override");
        }
        const run = async () => {
    ...
    async postDispatchOutput(channel, spec, text, error) {
        const label = spec.correlationId ? `${spec.id} · ${spec.correlationId}` : spec.id;
        try {
            if (error) {
                await this.sendResultCard(channel, "📨 Dispatch failed", `❌ ${error.slice(0, 1500)}`, 0xe74c3c);
            }
            ...
            if (chunks.length <= 3) {
                ...
                await this.sendResultCard(channel, "📨 Dispatch", `✅ Done — full output attached (${body.length} chars).`, DISPATCH_COLOR);
     ...
     *  inline as before. */
    async partitionAndStageAttachments(attachments, agentHasVision) {
    ...
    async sendResultFile(channel, name, body, prefix = "scheduled") {
            ...
            if (customId === "sessions:prev") {
                ...
                void (async () => {
                    ...
                    try {
                        ...
                        this.logger.info({ channelId: i.channelId, msgCount: rawMessages.length,
                            transcriptChars: sanitizedTranscript.length, model: compactionModel }, "rebuild: transcript assembled");
                        ...
                        const transcriptFile = path.join(cwd, `.rebuild-transcript-${i.channelId}-${Date.now()}.txt`);
                        await fsp.writeFile(transcriptFile, sanitizedTranscript, "utf8");
                        const compactionPrompt = `${fullTemplate}\n\n` +
                    ...
                    void (async () => {
                        ...
                        try {
                            ...
                            const successEmbed = new EmbedBuilder()
                                ...
                                .addFields({ name: "Chunks", value: String(result.stats.chunks), inline: true })
                                .setColor(0x2ecc71);
                            await btnInteraction.editReply({
                                embeds: [successEmbed],
                                components: [backRow],
                    ...
                    void (async () => {
                        ...
                        try {
                            ...
                            const successEmbed = new EmbedBuilder()
                                ...
                                .setDescription(`Compacted from Discord thread history into a **new session** \`${newId}\` with the multi-agent pipeline.` +
                                (wasActive ? ` This thread is now bound to it.` : ``) +
                                `\nOriginal \`${session.sessionId}\` is **preserved** (review or delete it from this list).`)
                                .addFields({ name: "Chunks", value: String(result.stats.chunks), inline: true })
                                .setColor(0x2ecc71);
    ...
    async cmdPresetBuilder(i, existing) {
        ...
        const render = () => {
            ...
            const agentSelect = new StringSelectMenuBuilder()
                ...
                .addOptions({ label: "Default", value: "__default__", default: state.agentId === null }, ...profiles.slice(0, 24).map((p) => ({
            ...
            const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder()
                ...
                .setStyle(ButtonStyle.Success)
                .setDisabled(!state.name), new ButtonBuilder()
     ...
     */
    async applyPresetToSession(channel, record, preset) {
        const changes = [];
        const notes = [];
        ...
        if (preset.effort) {
            ...
            if (supported.includes(preset.effort)) {
                ...
                notes.push(`⚠️ Effort \`${preset.effort}\` skipped — \`${record.agentId}\` bakes effort into the model choice.`);
            }
            else {
                notes.push(`⚠️ Effort \`${preset.effort}\` skipped — \`${record.agentId}\` has no settable reasoning effort.`);
            }
        }
        ...
        if (preset.instructions) {
            notes.push("ℹ️ Instructions are stored on the preset but not yet injected into the agent's prompt.");
        }

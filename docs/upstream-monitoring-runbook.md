...
>
> **Last updated**: 2026-09-06 (daily sweep)
>
...
- **Every print-mode spawn passes `--disable-slash-commands`** (requires agy >= 1.1.9; #47). Without it a prompt whose first token is an agy command or an installed skill name is resolved by the CLI instead of the model — verified on 1.1.25, `agy -p "/skills do X"` exits 2 and never starts a turn, on both the `-p` and the stdin path. Discord chat turns are already immune (`withHarnessPreamble` puts `<seam-harness>` first), so this covers the `injectTurn` callers that bypass the preamble: seam-mcp `handoff`/`forward`/`send`, cron and wake deliveries. Argv is built by the pure `buildAgyPromptArgs()` helper and regression-tested in `test/agy-prompt-args.test.ts`
...
- Also used by: [`agy.ts`](../packages/adapters/src/profiles/agy.ts) — `AgentSideConnection` for in-process ACP bridge

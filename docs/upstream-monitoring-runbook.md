...
>
> **Last updated**: 2026-08-22 (daily sweep)
>
...
- [ ] Check [Antigravity CLI releases](https://github.com/google-antigravity/antigravity-cli/releases) — last checked version: **1.1.18** (2026-08-22 sweep; installed = latest; 1.1.18: added project-name support for `--project`, `item.rename`/`item.delete` keybindings for conversation picker (f2/f4 rebindable), typo-tolerant `@` file completion fallback, improved audio attachment recognition (wav/mp3/m4a/aac/flac/opus); **fixed `-p` mode exiting exit-0 with empty response on dropped stream — now exits non-zero**; fixed valueless `--print` flag swallowing next flag; fixed `/btw` card pushing footer off-screen; fixed `/resume` workspace; fixed text styling after file link; fixed stray characters from input-mode re-arming; #42 updated)
...
- [ ] Check [Claude Code releases](https://github.com/anthropics/claude-code/releases) — last checked version: **2.1.240** (2026-08-22 sweep; installed 2.1.222, upgrade tracked in #40; 2.1.239: cost estimates include data-residency 1.1× premium, `/claude-api upgrade` for Python 0.x→1.x, fixed Bedrock streaming proxy stripping Content-Type (was doubling billed calls), fixed agentic-session race with Esc+queued prompt; 2.1.240: bug fixes; 2.1.236 notable: `ANTHROPIC_DEFAULT_MODEL` env var; no breaking seam-acp changes; #40 updated)
...
- [ ] Check [claude-agent-acp releases](https://github.com/agentclientprotocol/claude-agent-acp/releases) — last checked version: **0.70.0** (2026-08-22 sweep; installed 0.54.1, upgrade tracked in #39; no new releases since 2026-08-20 sweep)
...
#### GitHub / Copilot
- [ ] Check [Copilot CLI releases](https://github.com/github/copilot-cli/releases) — last checked version: **1.0.81-7 pre-release / 1.0.80 stable** (2026-08-22 sweep; installed 1.0.80 = stable current; 1.0.81-6: `defaultMode`/`defaultPermissionMode` settings, `--with-token` login flag, **ACP clients receive subagent IDs + raw event subscriptions + live title/mode/command/plan updates**, managed settings per-entry wins; 1.0.81-7: session restore on startup, `models.list` infoMessages/warningMessages, `copilot app` subcommand, voice dictation Ctrl+Space; no breaking seam-acp changes; #35 updated)
...
#### Opencode / LM Studio
- [ ] Check [opencode releases](https://github.com/anomalyco/opencode/releases) and npm [`opencode-ai`](https://registry.npmjs.org/opencode-ai/latest) — last checked version: **1.18.21** (2026-08-22 sweep; installed 1.15.13, upgrade tracked in #12; minor patch from 1.18.20; no config schema changes observed)
...
#### ACP Protocol
- [ ] Check [ACP SDK npm](https://registry.npmjs.org/@agentclientprotocol/sdk) — current pinned: `^1.1.0`, installed: 1.1.0, latest: **1.4.0** (up from 1.3.0 at last sweep; #28 scope updated to target 1.4.0) — last checked 2026-08-22

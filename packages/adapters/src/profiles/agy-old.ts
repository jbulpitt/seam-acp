/**
 * LEGACY Antigravity (`agy-old`) CLI adapter.
 *
...
export function makeAgyOldProfile(opts: {
  ...
  return asLocalAdapter({
    id: "agy-old",
    displayName: "Antigravity (legacy rollback only)",
    defaultModel,
    catalog: {
      ...
      async listSessions(cwd: string): Promise<SessionSummary[]> {
        try {
          ...
          try {
            ...
            try {
              const rows = db.prepare("SELECT acp_session_id FROM sessions WHERE repo_path = ? AND agent_id = ?").all(cwd, 'agy-old') as Array<{ acp_session_id?: string }>;

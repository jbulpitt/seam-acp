...
function completionToResponse(completion, model) {
  ...
  const output = [];
  if (reasoning) {
    output.push({ id: id('rs'), type: 'reasoning', summary: [{ type: 'summary_text', text: String(reasoning) }] });
  }
...
async function streamResponse(res, body, options) {
  ...
  const output = [];
  // Reasoning passthrough: Ollama streams the model's reasoning in delta.reasoning,
  // which codex renders as a thinking panel via reasoning_summary_* events.
  ...
  if (reasoningStarted) {
    const reasoningItem = { id: reasoningId, type: 'reasoning', summary: [{ type: 'summary_text', text: reasoningText }] };
    sse(res, 'response.reasoning_summary_text.done', { type: 'response.reasoning_summary_text.done', item_id: reasoningId, output_index: reasoningOutputIndex, summary_index: 0, sequence_number: sequence++, text: reasoningText });
    sse(res, 'response.reasoning_summary_part.done', { type: 'response.reasoning_summary_part.done', item_id: reasoningId, output_index: reasoningOutputIndex, summary_index: 0, sequence_number: sequence++, part: { type: 'summary_text', text: reasoningText } });
    sse(res, 'response.output_item.done', { type: 'response.output_item.done', output_index: reasoningOutputIndex, sequence_number: sequence++, item: reasoningItem });

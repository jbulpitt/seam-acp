...
export async function runPreflight(target, remoteScript, run = commandRunner) {
  ...
  if (report.verification_agent !== target.verifyAgent || (report.process_started_at !== "unknown" && !Number.isFinite(Date.parse(report.process_started_at ?? "")))) throw new Error("remote verification identity evidence is incomplete");

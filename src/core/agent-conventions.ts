...
export function harnessPreamble(opts?: { aliases?: ReadonlyArray<{ alias: string; description: string | null }> }): string {
  const bullets = [
  ...
  if (opts?.aliases && opts.aliases.length > 0) {
    ...
    bullets.push(
      `• To delegate work to another thread's agent, output a fenced code block whose info tag is \`seam-delegate:<alias>\` (e.g. \`\`\`seam-delegate:worker3). The body is the prompt to send. The bridge posts it as a message in the target thread, triggering that thread's agent. Available threads: ${list}.`
    );
  }
  return [
    "<seam-harness>",
...
export function withHarnessPreamble(userText: string, opts?: Parameters<typeof harnessPreamble>[0]): string {
  const body = userText ?? "";

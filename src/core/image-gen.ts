...
export type ImageProvider = "google" | "bfl";
...
export interface ImageModel {
  /** Internal stable id used in select-menu values, state, etc. */
  id: string;
  /** API model id sent to the provider. */
  apiId: string;
  /** Human label shown in the picker. */
  displayName: string;
  /** Short tagline shown in the select-menu description. */
  description: string;
  provider: ImageProvider;
  aspectRatios: AspectRatio[];
  /** Resolutions this model supports. */
  resolutions: Resolution[];
  /** Max number of reference images this model accepts; 0 = none. */
...
export interface GenerateResult {
  ...
  /** Provider-side message ID or generation ID, if available. */
  generationId?: string;
}
...
async function generateWithGeminiImage(
  ...
  const results = await Promise.all(
    Array.from({ length: count }, () => geminiOneShot(opts, apiKey))
  );
  return {
...
async function generateWithBfl(
  ...
  const results = await Promise.all(
    Array.from({ length: count }, () => bflOneShot(opts, apiKey))
  );
  return {
    images: results.flatMap((r) => r.images),
    ...(results[0]?.generationId ? { generationId: results[0]!.generationId } : {}),
...
async function bflOneShot(
  ...
  const submit = (await res.json()) as { id?: string; polling_url?: string };
  ...
  while (Date.now() - started < BFL_POLL_TIMEOUT_MS) {
    ...
    if (poll.status === "Ready" && poll.result) {
      const urls: string[] = poll.result.samples ?? (poll.result.sample ? [poll.result.sample] : []);
      if (urls.length === 0) throw new Error("FLUX returned no result URLs");
      const downloaded = await Promise.all(
        urls.map(async (u) => {
          const dl = await fetch(u);
          if (!dl.ok) throw new Error(`FLUX result download failed: ${dl.status}`);

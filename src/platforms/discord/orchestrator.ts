...
export class Orchestrator {
  ...
  private buildPresetListMessage(): {
    ...
    for (const p of presets.slice(0, 5)) {
      components.push(
        new ActionRowBuilder<ButtonBuilder>().addComponents(
          new ButtonBuilder().setCustomId(`pr:apply:${p.id}`).setLabel(`▶️ ${p.name}`.slice(0, 80)).setStyle(ButtonStyle.Success),
          new ButtonBuilder().setCustomId(`pr:edit:${p.id}`).setLabel("✏️ Edit").setStyle(ButtonStyle.Primary),
          new ButtonBuilder().setCustomId(`pr:del:${p.id}`).setLabel("🗑️ Delete").setStyle(ButtonStyle.Danger),
        )
      );
    }
    return { embeds: [embed], components };
  ...
  private async cmdPresetBuilder(
    ...
    const getModelsForAgent = (agentId: string | null): ReadonlyArray<{ modelId: string; name: string }> => {
      if (!agentId) return [];
    ...
    const render = () => {
      ...
      if (models.length > 0) {
        ...
        modelSelect.addOptions(
          { label: "Default (select an agent first for model list)", value: "__default__", default: true }
        );
      }
      ...
      // Buttons
      const buttons = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId("preset:details").setLabel("✏️ Name & details").setStyle(ButtonStyle.Primary),
        new ButtonBuilder().setCustomId("preset:tools").setLabel("🔧 Tools").setStyle(ButtonStyle.Secondary),
        new ButtonBuilder().setCustomId("preset:instr").setLabel("📋 Instructions").setStyle(ButtonStyle.Secondary),
        new ButtonBuilder()
          .setCustomId("preset:save")
          .setLabel(existing ? "💾 Save" : "✅ Create")
          .setStyle(ButtonStyle.Success)
          .setDisabled(!state.name),
        new ButtonBuilder().setCustomId("preset:cancel").setLabel("Cancel").setStyle(ButtonStyle.Secondary)
      );
      ...
      try {
        if (c.isStringSelectMenu() && c.customId === "preset:agent") {
          ...
          try {
            ...
            if (state.repoPath) {
              try { state.repoPath = resolveRepoPath(this.config.REPOS_ROOT, state.repoPath); } catch { /* keep raw */ }
            }
   ...
   */
  private async applyPresetToSession(record: SessionRecord, preset: Preset): Promise<string> {
  ...
  private async cmdPresetShow(i: ChatInputCommandInteraction): Promise<void> {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: "📋 Instructions", value: preset.instructions ? "```\n" + preset.instructions.slice(0, 1000) + "\n```" : "*(none)*" }
      )

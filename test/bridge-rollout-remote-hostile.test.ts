...
beforeAll(async () => {
  ...
  await fs.mkdir(path.dirname(entrypoint), { recursive: true }); await fs.mkdir(pm2Module); await fs.writeFile(entrypoint, "process.title='fixture-bridge';setInterval(()=>{},1000);\n");
...
describe.sequential("production remote shell deployment identity defenses (#241)", () => {
  it("accepts the exact bound process and rejects wrong app, UID, PID/cwd, and launcher", async () => {
    await expect(runRemote(["preflight"])).resolves.toMatchObject({ stdout: expect.stringContaining("identity_bound=yes") });

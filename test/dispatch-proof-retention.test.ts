...
const deps = () => ({ dataDir, logger, isArtifactDeletable: (id: string) => store.turnAttempts.get(id)?.deliveryDone === true });
...
describe("proof-only done retention (#306)", () => {
  it("retains terminal handoff/chain parents until the #305 resolver settles their actual onward child", async () => {
    ...
    store.updateDelegationStatus("next", "completed");
    expect(pruneDoneArtifact(canonicalDeps(), "chain").state).toBe("pruned");
  });

  it("retains unknown/nonterminal sources and expires only separately authorized legacy output", async () => {
    ...
    expect(pruneDoneArtifact(canonicalDeps(), "legacy").state).toBe("retained");
    store.authorizeDoneArtifactExpiration("legacy", "synthetic-operator", "explicitly expire this private fixture");
  ...
  it("uses the canonical resolver decision and does not log invalid private bodies", async () => {
    ...
    const warn = vi.fn();
    const configured = { ...canonicalDeps(), logger: { warn } as unknown as Logger };
    ...
    store.turnAttempts.markDeliveryDone("routed");
  ...
  it("prunes all delivered backlog pages without an age cutoff and retains unknown/undelivered files", async () => {
    ...
    const configured = { ...deps(), isArtifactDeletable: (id: string) => id.startsWith("resolved-") && deps().isArtifactDeletable("resolved") };
    ...
    for (let pass = 0; pass < 6; pass++) {
      ...
      expect(applied.pruned).toBeLessThanOrEqual(1000);
      totalPruned += applied.pruned;
      totalBytes += applied.bytes;
    }
    // Without a full yielding sweep, the measured backlog survives until many restarts.
    expect(totalPruned).toBe(count);
    expect(totalBytes).toBe(dry.bytes);
  ...
  it("preserves unresolved malformed artifacts and contains deletion to regular done files", async () => {
    ...
    const configured = { ...deps(), isArtifactDeletable: (id: string) => id !== "corrupt" };
  ...
  it("retries later delivery and stops before a closed-store lookup", async () => {
    ...
    const lookup = vi.fn(deps().isArtifactDeletable);
    const manager = new DoneRetention({ ...deps(), isArtifactDeletable: lookup }, 10);
    managers.push(manager);
    manager.start();
    await manager.drain();

...
These fixtures freeze the R1 behavioral contract and its R3 model-isolation
extension for Seam's primary, native `agy` adapter. They do not describe the
separately named optional `agy-package` adapter.
...
Its two synthetic model rows differ only by a baked effort label so tests can
observe literal, per-invocation `--model` bindings without a self-report.
